...
/// Read the slope gradient that the backward pass wrote into the context
fn slope_gradient(layer: &mut PReLU, ctx: &Ctx) -> Vec<f32> {
...
fn p_relu_forward_uses_1_slope_per_feature() {
    ...
    let got = layer.forward(&x, &mut Ctx::training()).unwrap();
...
fn p_relu_forward_broadcasts_a_channel_slope_over_space() {
    let layer = p_relu_with(vec![1, 2, 2, 2], vec![1, 2], vec![0.5, -0.25]);
    let x = tensor(
        &[1, 2, 2, 2],
...
fn p_relu_forward_leaves_0_alone_whatever_the_slope() {
    for alpha in [0.0f32, 0.25, -2.0, 100.0] {
        ...
        let got = layer
            .forward_mut(&tensor(&[1, 2], vec![0.0, -0.0]), &mut Ctx::training())
            .unwrap();
...
fn p_relu_with_a_zero_slope_matches_relu() {
    ...
    let mut learned_ctx = Ctx::training();
    let mut fixed_ctx = Ctx::training();

    assert_allclose(
        &learned.forward_mut(&x, &mut learned_ctx).unwrap(),
        &fixed.forward_mut(&x, &mut fixed_ctx).unwrap(),
        0.0_f32,
    );
    assert_allclose(
        &learned.backward(&g, &mut learned_ctx).unwrap(),
        &fixed.backward(&g, &mut fixed_ctx).unwrap(),
        0.0_f32,
    );
}
...
fn p_relu_with_a_uniform_slope_matches_leaky_relu_away_from_0() {
    ...
    let mut learned_ctx = Ctx::training();
    let mut fixed_ctx = Ctx::training();
    learned.forward_mut(&zero, &mut learned_ctx).unwrap();
    fixed.forward_mut(&zero, &mut fixed_ctx).unwrap();
    assert_eq!(
        learned.backward(&ones, &mut learned_ctx).unwrap()[[0, 0]],
        0.0
    );
    assert_eq!(fixed.backward(&ones, &mut fixed_ctx).unwrap()[[0, 0]], 1.0);
}
...
fn p_relu_slope_gradient_sums_over_the_batch() {
    ...
    assert_eq!(slope_gradient(&mut layer, &ctx), vec![-1.0, -5.0, -3.0]);
}
...
fn p_relu_slope_gradient_sums_over_every_shared_axis() {
    ...
    assert_eq!(slope_gradient(&mut layer, &ctx), vec![-6.0, -12.0]);
}
...
fn p_relu_slope_gradient_weights_by_the_upstream_gradient() {
    ...
    let mut ctx = Ctx::training();
    layer.forward(&x, &mut ctx).unwrap();
    layer.backward(&g, &mut ctx).unwrap();

    // Column 0: 2 * -1 plus -1 * -4. Column 1: 0.5 * -2, and 5 is positive so it adds nothing
    assert_eq!(slope_gradient(&mut layer, &ctx), vec![2.0, -1.0]);
}
...
fn p_relu_shared_axes_drops_a_stale_gradient() {
    ...
    let mut ctx = Ctx::training();
    layer
        .forward_mut(&Tensor::zeros(IxDyn(&[2, 3, 4])), &mut ctx)
        .unwrap();
    layer
    ...
    assert_ne!(
        layer.parameters_mut()[0].value.len(),
        stale,
...
fn p_relu_emits_c_order_tensors_from_a_strided_input() {
    ...
    let mut layer = PReLU::new(0.25).unwrap();
    let mut ctx = Ctx::training();
    let out = layer.forward_mut(&strided, &mut ctx).unwrap();
    assert!(out.is_standard_layout(), "forward output must be C order");

    let grad = layer.backward(&strided, &mut ctx).unwrap();
    ...
    assert_allclose(
        &twin.forward_mut(&packed, &mut twin_ctx).unwrap(),
        &out,
        0.0_f32,
    );
    assert_allclose(
        &twin.backward(&packed, &mut twin_ctx).unwrap(),
        &grad,
        0.0_f32,
    );
    assert_eq!(
        slope_gradient(&mut twin, &twin_ctx),
        slope_gradient(&mut layer, &ctx)
    );
}
...
fn p_relu_set_weights_repacks_a_strided_array() {
    ...
    let mut ctx = Ctx::training();
    layer
        .forward(&Tensor::ones(IxDyn(&[5, 2, 3])), &mut ctx)
        .unwrap();
    layer
        .backward(&Tensor::ones(IxDyn(&[5, 2, 3])), &mut ctx)
        .unwrap();
    assert_eq!(layer.parameters_mut()[0].value.len(), 6);
}
...
fn p_relu_parallel_path_matches_the_serial_path() {
    ...
    let mut layer = PReLU::new(0.25).unwrap().with_shared_axes(vec![1]).unwrap();
    let mut ctx = Ctx::training();
    let parallel_out = layer.forward_mut(&x, &mut ctx).unwrap();
    let parallel_grad = layer.backward(&g, &mut ctx).unwrap();
    ...
    for sample in 0..samples {
        ...
        let mut sample_ctx = Ctx::training();
        let serial_out = layer.forward(&xs, &mut sample_ctx).unwrap();
        let serial_grad = layer.backward(&gs, &mut sample_ctx).unwrap();
        assert_eq!(
...
fn p_relu_rejects_an_input_of_the_wrong_rank() {
    ...
    for shape in [vec![2usize, 3], vec![2, 3, 4, 1], vec![24]] {
        let err = layer
            .forward(&Tensor::ones(IxDyn(&shape)), &mut ctx)
            .unwrap_err();
        assert!(
            matches!(err, Error::InvalidInput(_)),
            "{shape:?} must be rejected, got {err:?}"
...
fn p_relu_accepts_any_extent_on_a_shared_axis() {
    let layer = p_relu_with(vec![1, 4, 4, 2], vec![1, 2], vec![0.5, -0.25]);
    let mut ctx = Ctx::training();
...
fn p_relu_accepts_any_batch_size() {
    ...
    for batch in [1usize, 3, 8, 40] {
        let out = layer
            .forward_mut(&Tensor::from_elem(IxDyn(&[batch, 3]), -4.0), &mut ctx)
            .unwrap();
...
fn p_relu_rejects_an_empty_input() {
    let mut layer = PReLU::new(0.25).unwrap();
    let err = layer
        .forward_mut(&Tensor::zeros(IxDyn(&[0, 3])), &mut Ctx::training())
        .unwrap_err();
...
fn p_relu_rejects_a_gradient_of_the_wrong_shape() {
    ...
    let mut ctx = Ctx::training();
    layer
        .forward_mut(&Tensor::ones(IxDyn(&[2, 3])), &mut ctx)
        .unwrap();
    let err = layer
        .backward(&Tensor::ones(IxDyn(&[2, 4])), &mut ctx)
        .unwrap_err();
...
fn p_relu_reports_its_type_and_output_shape() {
    ...
    // The build shape is what the layer reports
    assert_eq!(layer.output_shape(), "(None, 3, 4)");

    // A forward pass writes no shape, so a batch of 7 moves nothing
    layer
...
fn p_relu_trains_with_shared_spatial_axes() {
    ...
    assert_eq!(slope_gradient(&mut layer, &ctx).len(), 2);
}
