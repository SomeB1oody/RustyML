...
/// (this is the root cause of the previously flaky GRU parity test).
...
/// harness already spawns a fresh thread per test, but under `--test-threads=1` every test runs
/// on the same thread, so panic-safe cleanup is what actually makes the isolation hold there.
///
/// Always bind the guard to a variable; `GlobalSeedGuard::set(s);` (unbound) would drop
/// immediately and clear the seed before you use it — hence `#[must_use]`.
...
///
/// Use this for tensor comparisons. For single scalars, use approx's
