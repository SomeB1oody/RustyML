...
fn dense_2x2_with_weights(w_flat: Vec<f32>, b_flat: Vec<f32>) -> Dense {
    ...
    d.build(&Shape::known(&[1, 2])).unwrap();
...
fn dense_new_rejects_zero_dim() {
    let result = Dense::new(0, Linear::new());
    assert!(
...
fn dense_build_rejects_zero_input_dim() {
    ...
    let result = d.build(&Shape::known(&[1, 0]));
    assert!(
        matches!(result, Err(Error::InvalidInput(_))),
        "expected InvalidInput for input_dim=0, got {:?}",
        result
    );
}
...
fn dense_param_count_3x5() {
    use rustyml::neural_network::layers::ParamCounts;
    let mut d = Dense::new(5, Linear::new()).unwrap();
    d.build(&Shape::known(&[1, 3])).unwrap();
...
fn dense_forward_relu_zeroes_negative_preactivations() {
    let mut d = Dense::new(2, ReLU::new()).unwrap();
...
fn dense_forward_rank_3_matches_a_per_slice_reference() {
    ...
    let mut d = Dense::new(units, Linear::new()).unwrap();
    d.build(&Shape::known(&[batch, steps, features])).unwrap();
    ...
    reference.build(&Shape::known(&[1, features])).unwrap();
    reference
        .set_weights(
...
fn flatten_build_rejects_fewer_than_3_dims() {
    ...
    let result = layer.build(&Shape::known(&[4]));
    assert!(
        matches!(result, Err(Error::InvalidInput(_))),
        "expected InvalidInput for a rank-1 build shape, got {result:?}"
    );
}

/// A build shape that leaves an axis after the batch axis free is refused as well
#[test]
fn flatten_build_rejects_a_free_axis() {
    let mut layer = Flatten::new();
    let result = layer.build(&Shape::new(vec![None, None, Some(3)]));
    assert!(
        matches!(result, Err(Error::InvalidInput(_))),
        "expected InvalidInput for a free axis, got {result:?}"
    );
}

/// A rank-3 build shape is what the layer accepts
#[test]
fn flatten_build_accepts_a_rank_3_shape() {
    let mut layer = Flatten::new();
