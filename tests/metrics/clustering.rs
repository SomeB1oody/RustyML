...
fn test_ari_length_mismatch_panics() {
    let t = array![0isize, 0, 1];
    let p = array![0isize, 1];
...
fn test_ari_empty_panics() {
    let t: Array1<isize> = array![];
    let p: Array1<isize> = array![];
    let _ = adjusted_rand_index(&t, &p);
}
...
#[test]
fn test_fmi_range() {
    let t = array![0isize, 0, 0, 1, 1, 1];
    let p = array![0isize, 0, 1, 1, 1, 0];
...
fn test_silhouette_parallel_path_large_n() {
    ...
    let labels = Array1::from_shape_fn(n, |i| if i < n / 2 { 0isize } else { 1 });
...
fn test_silhouette_mislabeled_negative() {
    let x = array![[0.0, 0.0], [1.0, 0.0], [10.0, 0.0], [11.0, 0.0]];
    let labels = array![0isize, 1, 1, 0]; // p0 and p3 in cluster 0, p1 and p2 in cluster 1
...
fn test_silhouette_all_singletons_panics() {
    ...
    let labels = array![0isize, 1, 2]; // k = n = 3, invalid
