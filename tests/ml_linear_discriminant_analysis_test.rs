...
#[test]
fn test_lda_through_fit_predict_traits() -> Result<(), Box<dyn Error>> {
    // Now that LDA lives in `machine_learning`, it participates in the shared
    // estimator contract: it can be trained and queried through the generic
    // `Fit` / `Predict` traits, not only the inherent methods.
    let (x, y) = make_three_class_dataset();

    let mut lda = LDA::new(2, Some(Solver::SVD), Some(Shrinkage::Manual(0.1)))?;
    Fit::fit(&mut lda, (&x, &y))?;
    let predictions = Predict::predict(&lda, &x)?;
