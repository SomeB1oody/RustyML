...
- **6 new border layers, matching Keras 3: `ZeroPadding1D/2D/3D` and `Cropping1D/2D/3D`.** A zero-padding layer adds zero positions at the ends of the spatial axes, and a cropping layer removes them. Both leave the batch axis and the channel axis untouched, and neither holds a parameter. Each half is the backward pass of the other half, so a pad and a crop with the same amounts cancel. A `Cropping*` amount that leaves a spatial axis with no positions is `Error::InvalidInput` at forward time, on the same boundary Keras draws.
- **New `Border1D`, `Border2D`, and `Border3D` argument types.** Every border constructor takes `impl Into<..>`, and it returns the layer directly with no `Result`. A border amount is a `usize`, so no value is invalid at construction. The 3 forms match Keras. An integer sets every end. A tuple sets 1 equal amount per axis. A tuple of `(before, after)` pairs names every end on its own. Read the 2D and 3D tuple form with care. `ZeroPadding2D::new((1, 2))` gives 1 row at the top and the bottom, plus 2 columns at the left and the right. It does not give 1 row at the top and 2 at the bottom.
- **New `Identity` layer, matching Keras 3's `keras.layers.Identity`.** It passes its input through unchanged at every rank, and its backward pass returns the gradient it receives. `Identity::new()` takes no argument and returns the layer directly with no `Result`. Its use is a placeholder, for a builder that must return a layer when the choice is "no operation", and for a stack whose depth is a runtime value. It copies rather than borrows, because a layer returns an owned tensor.
- **New `Permute` layer, matching Keras 3's `keras.layers.Permute`.** `dims` names the new order of the axes after the batch axis. It counts from 1 and never includes the batch axis, as Keras writes it. The entries must be a permutation of `1..=dims.len()`, and the input rank must be `dims.len() + 1`. Unlike `Reshape`, a permute moves data rather than relabeling it, so the layer copies. The output is always in C order, so a consumer can read it as 1 contiguous slice. Keras accepts an empty `dims` and then serves a rank-1 input. `Permute::new(vec![])` returns `Error::InvalidParameter` instead, because such a layer reorders nothing.
- **New `RepeatVector` layer, matching Keras 3's `keras.layers.RepeatVector`.** It turns a `[batch, features]` input into `[batch, n, features]`, with the same vector at every step. The backward pass sums over the step axis. This bridges 1 recurrent layer to the next, because a recurrent layer here returns a rank-2 state and needs a rank-3 input. It is not Keras' `return_sequences=True`, which emits a different state at each step.
- **New `Reshape` layer, matching Keras 3's `keras.layers.Reshape`.** `target_shape` names the axes after the batch axis, which stays free, so 1 layer instance serves every batch size. At most 1 entry may be `-1`, and that axis takes the extent that makes the element count match. `Reshape::new(vec![-1])` is exactly `Flatten`, but unlike `Flatten` the constructor takes no input shape. The constructor rejects a `0` entry, which Keras defers to call time. The same programs stay valid, and only the moment of the error moves earlier.
- **New `UnitNormalization` layer, matching Keras 3's `keras.layers.UnitNormalization`.** It scales each group of elements to an L2 norm of 1, and the new `UnitNormalizationAxis` picks the axes the norm reduces over: `Default` for the last axis, `Custom(a)` for 1 named axis, and `Multiple(axes)` for a joint norm over several. It holds no parameter, learns no `gamma` or `beta`, and behaves the same in training and in inference, which makes it the first layer in `regularization` that is not mode-dependent. The scale is capped at `1e12`, Keras' `1 / epsilon` at its fixed `epsilon` of `1e-12`, so an all-zero group comes back all zero instead of dividing by zero. Keras accepts an empty axis list, and it builds a layer with a duplicate axis but then fails inside the reduction at the first call. `UnitNormalization::new` returns `Error::InvalidParameter` for both at construction.
- **`UnitNormalization` returns a real gradient where Keras returns `NaN`.** Keras builds its gradient by automatic differentiation, and the reciprocal square root it differentiates through overflows at a group the cap took over, so every element of such a group comes back `NaN`. This layer returns the derivative of the function it computed, which is the cap. The 2 agree everywhere else, checked over 106 shape and axis combinations and 57648 scalars against Keras 3.15.1.
