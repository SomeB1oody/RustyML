...
- **New `UnitNormalization` layer.** It scales each group of elements to an L2 norm of 1. The new `UnitNormalizationAxis` picks the axes the norm reduces over. `Default` names the last axis, `Custom(a)` names 1 axis, and `Multiple(axes)` names a joint norm over several axes. It holds no parameter and learns no `gamma` or `beta`. It behaves the same in training and in inference, which makes it the first layer in `regularization` that is not mode-dependent.
  - The scale is capped at `1e12`, so an all-zero group stays all zero instead of dividing by zero. `UnitNormalization::new` returns `Error::InvalidParameter` for an empty axis list or a duplicate axis, at construction.
...
- **New `Interpolation` argument on `UpSampling2D`, with 5 modes: `Nearest`, `Bilinear`, `Bicubic`, `Lanczos3`, and `Lanczos5`.** `Nearest` is the default, and it repeats each pixel into a block. The other 4 resample with a separable kernel, so a new pixel is a weighted sum of its neighbors along each axis. The weights of 1 output pixel always sum to 1, including at an edge where part of the kernel falls outside the image. A constant image therefore stays constant.
  - Only `UpSampling2D` takes this argument: `UpSampling1D` and `UpSampling3D` repeat and take no interpolation.
...
- **`UpSampling2D` computes its resample weight table in `f64`, not `f32`.** The Lanczos kernels have large lobes that cancel, so a `f32` table loses precision before it is ever applied. This layer rounds to `f32` once, at the end. Nearest and bilinear agree to the last bit that `f32` can hold.
- **`UnitNormalization` returns a real gradient at a group the cap took over.** The derivative of the reciprocal square root itself overflows at such a group, so differentiating through it directly would return `NaN` for every element. This layer returns the derivative of the function it computed, which is the cap. The 2 approaches agree everywhere else.
- **New `Embedding` layer, a trainable lookup table that turns whole-number indices into dense vectors.** `Embedding::new(input_dim, output_dim)` holds an `(input_dim, output_dim)` table and appends `output_dim` as a trailing axis, so `[N, T]` indices give `[N, T, output_dim]`. Any input rank of 1 or more works. The table starts from a uniform draw over `[-0.05, 0.05]`, the Keras default for this layer, and `.with_random_state(seed)` makes that draw reproducible. The backward pass adds the upstream gradient into the row that each index selected, in output order. A repeated index accumulates, and the result reproduces bit for bit.
  - Verified against Keras 3.15 with jax for the forward values and for the scatter-add gradient.
  - A `Tensor` holds `f32`, so an index arrives as a floating-point value. It is truncated toward zero and then range-checked, which is the rule the Keras cast to a whole number uses. An index outside `0..input_dim`, and any non-finite value, gives an `Error::InvalidInput` that names it. Keras leaves both cases to its backend, where an out-of-range index gives `NaN` and a negative index wraps to the end of the table.
...
- **New `LayerWeight::Embedding` variant and `EmbeddingLayerWeight` container, so an embedding table saves and loads.** The variant is appended at the end of the enum. postcard writes the variant index, so appending leaves every existing index alone, and a model file written before this release still loads. `MODEL_FORMAT_VERSION` therefore stays at 1.
- **The `Embedding` forward gather runs in parallel above the shared cheap-map gate** (`tuning::elementwise::set_cheap_map_f32`, 4,000,000 elements). A gather is a pure copy, so the gate never changes a value.
- **3 new transposed convolution layers: `Conv1DTranspose`, `Conv2DTranspose`, and `Conv3DTranspose`.** Each one runs a convolution backwards over its spatial axes, so it grows a tensor instead of shrinking one. This is the decoder half a generator or a segmentation head needs, and it completes the family the upsampling layers only approximated. The constructors mirror their plain counterparts: `Conv2DTranspose::new(filters, kernel_size, input_shape, strides, activation)`, plus `with_padding`, `with_random_state`, and `set_weights`. Output size is `input * stride + max(kernel - stride, 0)` under `PaddingType::Valid` and `input * stride` under `PaddingType::Same`, on each axis on its own. Verified against Keras 3.15 with jax, forward and backward.
  - The kernel is `[k..., filters, channels]`, with the filter axis **before** the input-channel axis. That is the reverse of the plain convolution kernel, because a transposed convolution reads channels and writes filters. `set_weights` therefore refuses a kernel laid out for the matching `Conv2D` whenever the filter count and the channel count differ. When the 2 counts are equal, the 2 layouts have the same shape, and no shape check can separate them.
  - Unlike the plain convolution layers, these put no lower bound on the input spatial size. A 1x1 input under a 3x3 kernel is a normal first decoder step, and `Conv2D::new` rejects exactly that shape.
  - There is no `output_padding` argument, so the round trip back to the original size is exact only under a condition on each axis. Under `PaddingType::Valid` the kernel must be at least the stride, and the stride must divide `input - kernel`. The round trip is short by the remainder otherwise. An 8-wide axis at kernel 3 and stride 2 convolves to 3 positions, and those 3 positions transpose back to 7, not 8. Under `PaddingType::Same` the stride must divide the input, and the round trip overshoots to the next multiple of the stride otherwise. Pick a stride that meets the condition, or resize afterwards with a border layer.
- **New `conv_transpose_engine`, a dimension-generic engine shared by all 3 layers.** A transposed convolution is the adjoint of a plain one, so the engine is the existing `convolution_engine` with its 2 halves exchanged. The forward pass is the plain backward's col2im scatter, and the backward pass is the plain forward's im2col gather. Every geometry helper, the offset table, and the block copy are reused unchanged, so 1 padding rule serves both engines. The pass parallelizes over batch items and reuses the existing `tuning::conv::set_parallel_min_flops` gate, because both engines run the same 2 GEMM shapes. The parallel path starts at the existing 4,000,000 default, and no new gate was added.
- **New `LayerWeight::Conv1DTranspose`, `LayerWeight::Conv2DTranspose`, and `LayerWeight::Conv3DTranspose` variants, with their weight containers, so a transposed kernel saves and loads.** The 3 variants are appended at the end of the enum, after `Embedding`. postcard writes the variant index, so appending leaves every existing index alone, and a model file written before this release still loads. `MODEL_FORMAT_VERSION` therefore stays at 1.
- **New `PReLU` layer, a ReLU whose negative-side slope is trainable.** `PReLU::new(input_shape, alpha)` holds 1 slope per position of `input_shape` with the batch axis removed, all starting at `alpha`. The forward transform is `x` for `x >= 0` and `alpha * x` below 0. An `alpha` of 0 starts the layer as a plain `ReLU`, and training moves each slope on its own. The slopes are ordinary trainable parameters, so `parameters()`, `get_weights()`, `set_weights(alpha)`, and `param_count()` all cover them.
  - Verified against Keras 3.15 with jax. The forward values and the input gradient agree to the last bit, and the slope gradient agrees to `f32` summation order.
  - **`with_shared_axes(axes)` cuts the slope count, and it is what makes the layer usable after a convolution.** Each named axis drops to extent 1 in the slope array and broadcasts back over the input. `[batch, height, width, channels]` with `[1, 2]` holds 1 slope per channel instead of 1 per pixel. Axis 0 is the batch axis and is always shared, so naming it is `Error::InvalidParameter`. The same applies to an axis at or above the input rank, or a repeated axis. A shared axis is also no longer checked at forward time, so 1 such layer serves images of several sizes.
  - **The derivative at exactly 0 is 0, and not 1 and not `alpha`.** That is neither branch, and it is what makes an `alpha` of 0 give the exact `ReLU` gradient as well as the exact `ReLU` transform. `LeakyReLU` differs here: it takes the positive branch at `x >= 0`, so its derivative at 0 is 1. A `PReLU` with a frozen uniform slope therefore matches `LeakyReLU` everywhere except at exactly 0.
  - **Decoupled weight decay skips the slopes.** Decay pulls a parameter toward 0. A slope of 0 turns the layer back into `ReLU`, so decay would erase what the layer learns. The slopes are exposed as a no-decay parameter, the same class as a bias and a normalization `gamma`.
...
- **New `LayerWeight::PReLU` variant and `PReLULayerWeight` container, so the slopes save and load.** The variant is appended at the end of the enum, after the transposed convolution variants. Every existing index stays put, and `MODEL_FORMAT_VERSION` stays at 1. The rank of the stored array follows the input rank, and a shared axis keeps its place in it at extent 1.
- **2 new convolution layers: `DepthwiseConv1D` and `SeparableConv1D`.** They complete the depthwise and separable family, which until now covered only 2D. A 1D encoder over a sequence can therefore use the same parameter-saving factorization a 2D one already could. The constructors mirror their 2D counterparts with scalar arguments instead of tuples: `DepthwiseConv1D::new(kernel_size, input_shape, stride, activation)` plus `with_depth_multiplier`, and `SeparableConv1D::new(filters, kernel_size, input_shape, stride, depth_multiplier, activation)`. Both take `with_padding`, `with_random_state`, and `set_weights`. The depthwise kernel is `[kernel_size, channels, depth_multiplier]`, and the pointwise kernel is `[1, channels * depth_multiplier, filters]`.
  - The kernels match Keras. Verified against Keras 3.15 with jax, forward and backward. Keras itself deviates from an `f64` reference on the same cases, so the residual is `f32` summation order and not a difference in meaning.
  - **Neither layer adds a loop nest.** A `[batch, length, channels]` tensor holds its values in the same order as `[batch, 1, length, channels]`. A `[kernel_size, channels, depth_multiplier]` kernel holds its values in the same order as `[1, kernel_size, channels, depth_multiplier]`. Both 1D layers set the height terms of the shared depthwise geometry to 1, and pass the flat slices of their own rank-3 arrays. Nothing is repacked, and the existing `tuning::conv::set_naive_parallel_min_flops` gate covers them with no new gate.
  - `SeparableConv1D` routes its pointwise stage through the shared im2col plus GEMM engine at spatial rank 1, the same way `SeparableConv2D` does at rank 2. A 1-tap convolution under `Valid` padding leaves the length alone, so the depthwise stage alone sets the output length.
- **New `LayerWeight::DepthwiseConv1D` and `LayerWeight::SeparableConv1D` variants, with their weight containers, so a 1D depthwise or separable kernel saves and loads.** The 2 variants are appended at the end of the enum, after `PReLU`, so every existing index stays put and `MODEL_FORMAT_VERSION` stays at 1.

### Fixed
- **`DepthwiseConv2D::output_shape` reported the input channel count instead of `channels * depth_multiplier`.** It read the trailing axis of the shared 2D output-shape calculator, which carries the input channel count through. That is right for a plain convolution, but not for a depthwise one. `model.summary()` therefore printed the wrong output width for any such layer above the default multiplier of 1, while the forward pass emitted the correct one. Only the printed description was wrong. No stored weight, forward value, or gradient changes, and a saved model still loads, because the load path never compares `output_shape`.
- **`SeparableConv2D` returned a panic, not an error, for an input carrying more channels than it was built for.** Its depthwise kernel is sized from the declared channel count, so the shared kernel indexed past the end of it. The layer died inside `forward` or `predict`. `DepthwiseConv2D` already rejected the same input with `Error::DimensionMismatch`. Both separable layers now check the rank and the channel count at the layer boundary, so the whole family reports this the same way. An input with the declared channel count is unaffected.
...
### Changed
- **The depthwise forward and backward driver loops moved into the shared `conv_op_helpers` module.** `DepthwiseConv2D` and `SeparableConv2D` each carried their own copy of the row-chunking forward loop and the per-batch-item backward reduction. The 2 new 1D layers would have made 4 copies. All 4 layers now call 1 `depthwise_forward` and 1 `depthwise_backward`. The parallel gate, the task split, and the batch-order reduction are unchanged, so every value is the same as before.
