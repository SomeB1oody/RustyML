...
### Added
- **Public deterministic blocked reductions: `math::reduction`.** The crate-internal `reduction` module moves into `math` as a public API: `det_par_fold` folds a `&[f64]` slice and the new `det_par_fold_range` folds an index range `0..n`, for reductions that zip several arrays or accumulate compound state. Both fold fixed `DET_REDUCE_BLOCK` blocks and merge in block order, so the result is deterministic at any thread count — and 2-3x faster than a bare rayon reduction on L3-resident inputs.
...
- `math::logistic_loss` runs as a deterministic blocked parallel reduction above a measured exp-class gate. **Behavior change:** above the gate the summation grouping changes, so results differ in the last bits from previous versions. End-to-end, `logistic_fit_50000x64_100it` drops 120 ms → 105 ms (~12%).
