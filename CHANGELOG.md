...
- **`LogisticRegression::fit` and `fit_predict` accept `x` and `y` with different storage types** (independent `S1`/`S2` parameters), so an owned matrix can be paired with a borrowed label view. Purely a relaxation — existing calls still compile.
