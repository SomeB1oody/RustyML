...
- **New `Embedding` layer, a trainable lookup table that turns whole-number indices into dense vectors.** `Embedding::new(input_dim, output_dim)` holds an `(input_dim, output_dim)` table and appends `output_dim` as a new trailing axis, so an `[N, T]` batch of token indices gives `[N, T, output_dim]`. Any input rank of 1 or more works. The table starts from a uniform draw over `[-0.05, 0.05]`, the Keras default for this layer, and `.with_random_state(seed)` makes that draw reproducible. The backward pass adds the upstream gradient into the row that each index selected, in output order, so a repeated index accumulates and the result reproduces bit for bit. Verified against Keras 3.15 with jax for the forward values and for the scatter-add gradient.
  ...
  - `mask_zero` has no counterpart, because this crate propagates no mask. The flag would change nothing, so it is left out rather than accepted and ignored.
...
- **New `PReLU` layer, a ReLU whose negative-side slope is trainable.** `PReLU::new(input_shape, alpha)` holds 1 slope per position of `input_shape` with the batch axis removed, all starting at `alpha`. The forward transform is `x` for `x >= 0` and `alpha * x` below 0, so an `alpha` of 0 starts the layer as a plain `ReLU` and training moves each slope on its own. The slopes are ordinary trainable parameters, so `parameters()`, `get_weights()`, `set_weights(alpha)`, and `param_count()` all cover them. Verified against Keras 3.15 with jax over 22 rank, shared-axis, and slope combinations: the forward values and the input gradient agree to the last bit, and the slope gradient agrees to `f32` summation order.
  - **`with_shared_axes(axes)` cuts the slope count, and it is what makes the layer usable after a convolution.** Each named axis drops to extent 1 in the slope array and broadcasts back over the input, so `[batch, height, width, channels]` with `[1, 2]` holds 1 slope per channel instead of 1 per pixel. Axis 0 is the batch axis and is always shared, so naming it is `Error::InvalidParameter`, as is an axis at or above the input rank or a repeated axis. A shared axis is also no longer checked at forward time, so 1 such layer serves images of several sizes.
  ...
  - **Decoupled weight decay skips the slopes.** Decay pulls a parameter toward 0, and a slope of 0 turns the layer back into `ReLU`, so decay would erase what the layer learns. The slopes are exposed as a no-decay parameter, the same class as a bias and a normalization `gamma`.
...
- **New `LayerWeight::PReLU` variant and `PReLULayerWeight` container, so the slopes save and load.** The variant is appended at the end of the enum, after the transposed convolution variants, so every existing index stays put and `MODEL_FORMAT_VERSION` stays at 1. The rank of the stored array follows the input rank, and a shared axis keeps its place in it at extent 1.
- **2 new convolution layers: `DepthwiseConv1D` and `SeparableConv1D`.** They complete the depthwise and separable family, which until now covered only 2D, so a 1D encoder over a sequence can use the same parameter-saving factorization a 2D one already could. The constructors mirror their 2D counterparts with scalar arguments in place of tuples: `DepthwiseConv1D::new(kernel_size, input_shape, stride, activation)` plus `with_depth_multiplier`, and `SeparableConv1D::new(filters, kernel_size, input_shape, stride, depth_multiplier, activation)`. Both take `with_padding`, `with_random_state`, and `set_weights`. The kernels match Keras: `[kernel_size, channels, depth_multiplier]` for a depthwise stage and `[1, channels * depth_multiplier, filters]` for a pointwise one. Verified against Keras 3.15 with jax over 11 shape, stride, padding, and multiplier combinations, forward and backward, at a worst relative deviation of `1.4e-7`. Keras itself deviates by `8.2e-8` from an `f64` reference on the same cases, so the residual is `f32` summation order and not a difference in meaning.
  - **Neither layer adds a loop nest.** A `[batch, length, channels]` tensor holds its values in the same order as `[batch, 1, length, channels]`, and a `[kernel_size, channels, depth_multiplier]` kernel in the same order as `[1, kernel_size, channels, depth_multiplier]`. Both 1D layers therefore set the height terms of the shared depthwise geometry to 1 and hand over the flat slices of their own rank-3 arrays. Nothing is repacked, and the existing `tuning::conv::set_naive_parallel_min_flops` gate covers them with no new gate.
...
### Fixed
- **`DepthwiseConv2D::output_shape` reported the input channel count instead of `channels * depth_multiplier`.** It read the trailing axis of the shared 2D output-shape calculator, which carries the input channel count through, and that is right for a plain convolution but not for a depthwise one. `model.summary()` therefore printed the wrong output width for any such layer above the default multiplier of 1, while the forward pass emitted the correct one. Only the printed description was wrong. No stored weight, forward value, or gradient changes, and a saved model still loads, because the load path never compares `output_shape`.
- **`SeparableConv2D` returned a panic, not an error, for an input carrying more channels than it was built for.** Its depthwise kernel is sized from the declared channel count, so the shared kernel indexed past the end of it, and the layer died inside `forward` or `predict`. `DepthwiseConv2D` already rejected the same input with `Error::DimensionMismatch`. Both separable layers now check the rank and the channel count at the layer boundary, so the whole family reports this the same way. An input with the declared channel count is unaffected.
...
### Changed
- **The depthwise forward and backward driver loops moved into the shared `conv_op_helpers` module.** `DepthwiseConv2D` and `SeparableConv2D` each carried their own copy of the row-chunking forward loop and the per-batch-item backward reduction, and the 2 new 1D layers would have made 4 copies. All 4 layers now call 1 `depthwise_forward` and 1 `depthwise_backward`. The parallel gate, the task split, and the batch-order reduction are unchanged, so every value is the same as before.
