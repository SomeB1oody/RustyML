...
## [Unreleased] - 2026-06-14 (UTC-7)
### Added
- **`AdamW` optimizer: Adam with decoupled weight decay** (`neural_network::optimizers::AdamW`). Same adaptive-moment math and bias correction as `Adam`, but a non-zero `weight_decay` shrinks the parameter directly by `(1 - learning_rate * weight_decay)` before the gradient step (the Loshchilov & Hutter decoupled formulation) rather than being folded into the gradient — generally the better-behaved choice with adaptive optimizers. It honors the same weights-only exclusion (biases and normalization `gamma`/`beta` are never decayed) and the same `with_clip_norm` builder as the other optimizers. `Adam` and `AdamW` share a private `AdamCore` holding the common state, validation, clip-norm, bias-correction timestep, and lazily-sized moment buffers, so the only difference between the two is a `decoupled` flag.

### Changed
- **Breaking: `Adam`'s `weight_decay` is now classic coupled L2 regularization, not decoupled.** `Adam` previously applied *decoupled* (AdamW-style) weight decay; that behavior now lives in the new `AdamW` optimizer. `Adam`'s `weight_decay` now folds an L2 term `weight_decay * param` into the gradient (via the new `l2_regularized_grad` kernel), so it flows through the moment estimates and is rescaled by the adaptive `1 / (sqrt(v_hat) + epsilon)` denominator — matching PyTorch's `Adam(weight_decay=...)`. **Behavior changes only for `Adam` with `weight_decay > 0`**; with `weight_decay == 0.0` (the default) `Adam` is unchanged and bit-identical to `AdamW`. Code relying on the old decoupled behavior should switch to `AdamW`. Both still honor the weights-only exclusion (biases / norm `gamma`/`beta` are not decayed).
- `Sequential::fit_with_batches` no longer keeps two near-identical copies of its epoch/batch training loop (one per `show_progress` feature state). The shuffle → `chunks(batch_size)` → `create_batch_tensors` → `train_batch` logic is now written once, with only the progress-bar bookkeeping (`epoch_loss`/`batch_count`/`set_message`/`inc`) gated behind `#[cfg(feature = "show_progress")]`. Removes the drift risk where a batching-logic fix could land in one copy and not the other; no behavior change in either feature configuration.
- **Breaking: weight decay now applies only to weight tensors, excluding biases and normalization scale/shift (`gamma`/`beta`), matching the PyTorch/Keras convention.** Every optimizer (`SGD`, `AdaGrad`, `RMSprop`, `Adam`) previously called `apply_weight_decay` on *every* tensor a layer yielded, so a non-zero `weight_decay` also shrank biases and BatchNorm/LayerNorm/InstanceNorm/GroupNorm `gamma`/`beta` — generally considered mildly harmful and not what those frameworks do. The per-parameter view `ParamGrad` gains a `decays: bool` field and two constructors — `ParamGrad::weight(value, grad)` (`decays = true`, for dense/conv/recurrent kernels) and `ParamGrad::no_decay(value, grad)` (`decays = false`, for biases and norm `gamma`/`beta`) — and each layer's `parameters()` tags its tensors accordingly; the optimizers gate `apply_weight_decay` on `pg.decays`. **Breaking for code that constructs `ParamGrad` directly** (the struct now has a third field; use the constructors). **No numerical change when `weight_decay == 0.0`** (the default) or for the gradient step and clip-by-global-norm, which still treat every parameter identically — only the weight-decay term of an opted-in run changes, and only for biases/norm parameters (which are no longer shrunk). Two differential tests pin the invariant: a Dense layer's bias is byte-identical with and without decay while its weights shrink by exactly `lr*wd*w0`, and a BatchNorm layer's `gamma`/`beta` update is byte-identical with and without decay.
- `CategoricalCrossEntropy`'s `from_logits` path drops two redundant `to_owned()` clones of the reshaped one-hot labels: `&labels.to_owned() * &log_sm` and `&sm - &labels.to_owned()` become `&labels * &log_sm` and `&sm - &labels`, multiplying/subtracting straight through the `CowArray` view (ndarray's reference arithmetic accepts a `Cow` operand directly). Saves one `[batch, classes]` allocation each in `compute_loss` and `compute_grad`; no behavior change.

### Fixed
- **`MeanAbsoluteError` now propagates NaN through its gradient instead of silently zeroing it.** `compute_grad` mapped `p - t` to its scaled sign `{+1, -1, 0} / n`, but a NaN difference is neither `> 0` nor `< 0`, so it fell into the zero branch — reporting a finite (zero) gradient while `compute_loss` already returned NaN (`|NaN| = NaN`), an internal forward/backward asymmetry that masked the upstream numerical problem and diverged from numpy/PyTorch `sign(NaN) = NaN`. The exact-zero tie is now matched explicitly (`x == 0.0`) and the remaining `else` — only reachable for NaN — returns `f32::NAN`. This is the same class of silent-NaN masking just fixed for ReLU and max pooling; the other four losses already propagate NaN (MSE's `2x / n`, and BCE/CCE/SCCE's `clamp`, which returns NaN for a NaN input). **Behavior change for NaN inputs only**; every finite input's gradient is bit-identical.
...
- **Breaking: the same constructor refactor extends to `neural_network` layers and optimizers — `Option`/strategy-default parameters move from `new` to chainable `with_*` builders, so `new` keeps only the layer's defining shape/hyperparameters (the Keras-required ones).** Same split rule as the `machine_learning`/`utils` pass. Moved settings: every weight/mask/noise-bearing layer's `random_state` (`Dense`, `Conv1D`/`Conv2D`/`Conv3D`/`DepthwiseConv2D`/`SeparableConv2D`, `SimpleRNN`/`GRU`/`LSTM`, `Dropout`/`SpatialDropout1D`/`2D`/`3D`, `GaussianDropout`/`GaussianNoise`) → `with_random_state`; the pooling layers' optional `stride`/`strides` → `with_stride`/`with_strides` (still defaulting to `pool_size`) and their `padding` → `with_padding`; the conv layers' `padding` → `with_padding`; `LayerNormalization`'s `normalized_axis` → `with_normalized_axis`; and every optimizer's `clip_norm` (`SGD`, `AdaGrad`, `RMSprop`, `Adam`) → `with_clip_norm`. So e.g. `Conv2D::new(32, (3,3), shape, (1,1), ReLU).with_padding(Same).with_random_state(42)`, `MaxPooling2D::new((2,2), shape).with_strides((2,2))`, and `Adam::new(0.001, 0.9, 0.999, 1e-8, 0.0).with_clip_norm(1.0)`. Stride/clip-norm/axis setters validate and return `Result<Self, Error>` (fail-fast preserved); `with_random_state`/`with_padding` return `Self`. Each `new` gains a `# Notes` section listing its `with_*` methods. `with_random_state` on a weight-bearing layer re-runs that layer's exact initialization with the given seed, so `Layer::new(..).with_random_state(s)` produces byte-identical weights to the old `Layer::new(.., Some(s))`; **behavior is numerically unchanged** for any fixed seed (the one subtlety: when a thread-global seed *and* `with_random_state` are both in use, `new`'s default draw now advances the global stream once before the override, which can shift a later *unseeded* layer's sub-seed — still deterministic, just not bit-identical to the old version in that mixed case). `PaddingType` and `LayerNormalizationAxis` now `#[derive(Default)]` on their canonical variant (`Valid`, `Default`).
...
- **SpatialDropout1D/2D/3D apply their per-channel mask as a single fused scale pass and never materialize a full-size mask.** These layers sample one keep/drop value per `(batch, channel)` and previously (a) expanded it to a full `[batch, channels, *spatial]` mask — first with a per-element nested-loop fill doing `ArrayD` dynamic indexing, the dominant serial cost — then (b) multiplied `input * mask * scale` in two passes, and (c) stored that full-size mask for backward. The forward and backward now run one fused pass over the contiguous `[batch, channels, *spatial]` data: each `(batch, channel)` is a contiguous `spatial`-length segment, so the shared `spatial_dropout_scale` helper scales segment `i` by `channel_mask[i] / (1 - rate)` straight into the output, broadcasting the per-channel value implicitly. Only the small `[batch, channels]` mask is stored (was input-sized — e.g. 8KB instead of 33MB at conv scale), and backward reuses the same helper on the gradient. Each element is independent (no reduction), so the work splits into per-segment tasks gated by the measured `SPATIAL_DROPOUT_SCALE_PARALLEL_MIN_ELEMS` (cheap-map class, crossover 1M-4M elements -> `4_194_304`); the flag never changes the bits, and the whole pass is **bit-for-bit identical** to the previous `input * broadcast(mask) * scale` (the mask is binary, so `(x*1)*scale == x*(1*scale)` and `(x*0)*scale == x*(0*scale)` exactly). All existing channel-consistency / backward tests pass unchanged; a new flag-invariance unit test pins serial == parallel == the explicit two-step form. New `nn_end_to_end` trackers (`spatial_dropout_{1d,2d,3d}_forward`, `spatial_dropout_{2d,3d}_backward`) and a `parallel_gates` calibration section measure it: at 8.4M-element conv scale the forward drops ~12-14x overall (1D 25.6 -> 2.4 ms, 2D 29.4 -> 2.4 ms, 3D 33.0 -> 2.4 ms) and the backward ~3x (2D/3D 6.9 -> 2.4 ms).
- `GaussianDropout::backward` borrows its cached forward noise (`Option::as_ref`) instead of consuming it (`Option::take`), so backward is now idempotent — calling it twice after one forward returns the same gradient both times, matching the other dropout layers (which read `&self.mask`). The next training forward overwrites the cache as before; no behavior change for the normal one-forward-one-backward training loop.
- `softmax` backward borrows a contiguous `[batch, features]` view of its read-only `output` and `grad_output` operands (via `to_shape`) instead of always cloning them (`to_owned`), cloning only when the source is non-standard-layout. No behavior change; the forward still owns its reshaped buffer since it transforms it in place.
- **`DepthwiseConv2D` and `SeparableConv2D`'s depthwise stage now convolve directly over flat contiguous buffers, dropping the per-position temporaries and 4-D dynamic indexing of the old loop nests.** The per-channel kernels (`DepthwiseConv2D::convolve_channel` / `compute_channel_gradients`, `SeparableConv2D::compute_depthwise_channel` and the depthwise backward) previously either allocated a fresh `input_patch * kernel` array for every output position (millions of tiny allocations per pass) or indexed a 4-D `ArrayD` per element (`input[[b, c, i, j]]`). They now slice the channel and kernel to `&[f32]` once and run a multiply-accumulate over the kernel window with plain offset arithmetic — same math, no allocations, no bounds-checked dynamic indexing. Measured at MobileNet scale (`[8, 64, 56, 56]`, 3x3, `Same`): depthwise forward **-74%** (5.7 -> 1.5 ms), depthwise forward+backward **-47%** (8.7 -> 4.7 ms); separable forward **-16%** (2.6 -> 2.2 ms), forward+backward **-30%** (9.4 -> 6.6 ms). Results are numerically equivalent (the backward fuses the weight- and input-gradient passes, so floats accumulate in a different order) — validated by the finite-difference gradient checks and the known-value / identity / `dm=2` tests. Also folds `DepthwiseConv2D::forward`/`::predict`, which were ~95 near-identical lines each, into a shared `convolve` helper that writes caches only on success. (Two alternatives were benchmarked and rejected first: routing depthwise through the dense im2col + GEMM engine via a `groups` parameter — a net 1.3-2.2x *slowdown*, since depthwise is a degenerate `M == 1` GEMM per group where im2col materialization is pure overhead — confirming the direct-convolution approach, as in production libraries that ship dedicated depthwise kernels.)
- The three engine-backed conv layers report a missing input cache in `backward` consistently as `Error::forward_pass_not_run("Conv{1,2,3}D")` (Conv1D and Conv3D previously raised ad-hoc `Error::computation` messages, Conv2D already used this). This branch is unreachable in practice — `output_cache.take()` succeeding already proves the forward ran and set the input cache in the same call — so the change is cosmetic; it just aligns the three layers with each other and with the depthwise/separable layers.
- **`SimpleRNN`, `LSTM`, and `GRU` no longer duplicate their timestep recurrence between `forward` and `predict`** — each had the full per-timestep loop (~50 lines for LSTM/GRU) written twice, differing only in whether it recorded backward caches, so a fix to one path could silently diverge from the other (a train/inference mismatch). Both now delegate to a shared private `run(&self, x3, caches: Option<&mut Caches>)`: `forward` passes `Some` and records the per-timestep values, `predict` passes `None` and skips the recording and its clones. As part of this the per-timestep caches collapse from many separate `Option<Vec<_>>` layer fields into a single `Option<LstmCaches>` / `Option<GruCaches>` (`SimpleRNN` keeps its lone hidden-state vec), which the backward destructures in one move. No behavior change — the `*_predict_equals_forward`, finite-difference gradient, and serialization round-trip tests pass unchanged; these are runtime caches, so the on-disk format is unaffected. Net ~80 fewer lines of drift-prone numeric code.
...
### Fixed
- `GaussianNoise::new` now rejects a non-finite `stddev` (NaN / +inf) with `Error::InvalidParameter` at construction. Previously the stddev validator only checked for negativity, so a non-finite value passed construction and then reached `Normal::new(0.0, stddev).unwrap()` in the forward pass — which rejects non-finite `std_dev` and **panicked** (an uncaught panic, deferred to the first forward). The validator now also requires finiteness, matching `validate_epsilon`'s contract and failing fast. `GaussianDropout` was never affected (its stddev `sqrt(rate / (1 - rate))` is finite for the validated `rate` in `[0, 1)`).
- **ReLU now propagates NaN instead of silently mapping it to 0.** The `Activation::ReLU` forward (used by the `ReLU` layer and embedded in `Dense`/conv/recurrent) was `if x > 0 { x } else { 0 }`, so a NaN input — which is not `> 0` — was silently zeroed, masking upstream numerical problems (exploding gradients, bad data) and diverging both from the sibling activations (sigmoid/tanh/softmax all propagate NaN) and from PyTorch/TensorFlow/NumPy. It is now `if x <= 0 { 0 } else { x }`, so NaN takes the `else` branch and propagates. This also removes a forward/backward asymmetry: backward (`if a <= 0 { g = 0 }`) already passed a NaN activation's gradient through, so forward and backward now treat NaN consistently. **Behavior change for NaN inputs only** (`ReLU(NaN)`: 0 → NaN); every finite and infinite input is bit-identical (`ReLU(+inf) = +inf`, `ReLU(-inf) = 0`, `ReLU(0) = 0`).
- **Max pooling now propagates NaN instead of silently dropping it** (`MaxPooling1D/2D/3D`, `GlobalMaxPooling1D/2D/3D`). The pooling engine's max reduction was `if v > max_val { ... }`, and since `NaN > x` is always false, a window (or, for global pooling, a channel plane) containing a NaN returned the maximum of the *other* elements — silently discarding the NaN, the same class of masking just fixed for ReLU and diverging from PyTorch/TensorFlow/NumPy, whose max pooling propagates NaN. The reduction now takes a NaN as the window result and records its position as the arg-max (so the backward gradient routes to it, matching the frameworks); once a NaN is seen it sticks. The strict-`>` first-maximum tie-break for finite values is unchanged, and average pooling was never affected (its running `sum += v` already propagates NaN). **Behavior change for NaN inputs only**; every finite/infinite input is bit-identical.
