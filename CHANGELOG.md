...
- **New `Permute` layer, matching Keras 3's `keras.layers.Permute`.** `dims` names the new order of the axes after the batch axis. It counts from 1 and never includes the batch axis, as Keras writes it. The entries must be a permutation of `1..=dims.len()`, and the input rank must be `dims.len() + 1`. Unlike `Reshape`, a permute moves data rather than relabeling it, so the layer copies. The output is always in C order, because `Activation::Softmax` rejects a tensor that is not. Keras accepts an empty `dims` and then serves a rank-1 input. `Permute::new(vec![])` returns `Error::InvalidParameter` instead, because such a layer reorders nothing.
