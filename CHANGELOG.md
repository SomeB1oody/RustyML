...
## [Unreleased]
### Added
- **`SequentialBuilder`, and it is the only way to reach a `Sequential`.** The builder collects layers, and `build(&input_shape)` walks the stack once from the input, gives every layer the shape that reaches it, and threads each output shape into the next layer. `add` never fails and it chains, so a whole stack reads as 1 expression. `new_with_seed` presets the fit-time shuffle seed of the built model.
  - `fit`, `fit_with_batches`, `train_batch`, `evaluate`, `predict`, `summary`, `save_to_path`, `load_from_path`, `load_partial_from_path`, `weight_paths`, and `weight` are methods of the built model alone. Training a model that was never built is therefore a compile error, and no run-time state says whether a model is ready.
  - `build` refuses a stack whose shapes do not agree, and the message names the position of the layer and its type: ``invalid input: layer 1 (`MaxPooling2D`) refused the input shape (4, 8): ...``. Such an error used to appear in the middle of a forward pass, deep inside a model, with no layer named. Nothing is allocated past the position that refused.
  - `Sequential::input_shape()` gives the shape the model was built for, and `Sequential::output_shape()` gives the shape it produces for that input.
  - `SequentialBuilder` joins `Sequential` and `History` in the `neural_network` prelude, next to the `Shape` that `build` takes, so `use rustyml::prelude::*` covers a whole model from the first `add` to `predict`.
- **`Layer::build` and `Layer::compute_output_shape`, and the `Shape` type that both speak.** `build(&mut self, &Shape)` allocates every array the layer owns, from the shape of its input. `compute_output_shape(&Shape)` is pure: it reads the layer configuration alone, so it answers on a layer that was never built and never ran, and it gives the same answer after any number of passes. `Layer::output_shape` is derived from it, against the shape the layer holds.
  - `Shape` holds 1 entry per axis, and each entry is a fixed extent or a free one. It prints the way a summary always printed a shape. The batch axis is free in most layers, because 1 layer serves every batch size, and a recurrent layer that returns a sequence leaves its time axis free as well.
  - `Shape::new`, `Shape::known`, `Shape::with_free_batch`, and `Shape::from_batch` build one. `rank`, `axes`, `dims`, `free_batch`, `check_rank`, `check_min_rank`, and `split_batch` read one. `Shape` is in the `neural_network` prelude.
  - `NnError::NotBuilt` is a 5th variant of the neural-network error. `Layer::predict` takes `&self` and cannot build, so it is the path that reports an unbuilt layer. `Layer::forward` builds from the tensor it receives instead.
- **`Initializer`, the value that holds the rules of a weight draw, and the `Fans` pair a layer supplies with it.** Every drawn array of the neural-network module comes from this closed set of 3 rules: `GlorotUniform` for every kernel, `Uniform { limit }` for the embedding table, and `Orthogonal` for the 3 recurrent kernels. It is `Copy`, it compares by value, and it serializes, so a layer holds it by value.
  - The layer builds the `Fans` pair from its own configuration, and no function of the module derives a fan from the shape of the array. A transposed convolution stores its kernel with the filter axis before the input-channel axis, so a rule that reads the last 2 axes gives those 3 layers the swapped pair. Glorot reads the sum of the 2 fans and the sum is symmetric, so the defect stays invisible until an initializer that reads 1 fan alone arrives.
  - No drawn weight moved. Every layer draws the same values, from the same seed, in the same order.
...
- **The checkpoint records the shape each layer was built for, in a `BuildConfig` field.** A layer allocates every array it owns in `Layer::build`, from the shape of its input, so the build shape decides every extent the layer allocates. The record frees the batch axis, because 1 layer serves every batch size, so a model built for 32 samples takes the checkpoint of a model built for 1. A load compares the field only when the file and the layer both carry a shape, and a layer that owns no array and reads no extent carries none.
  - A convolution is what the field is for. A `Conv2D` kernel is `[kh, kw, Cin, filters]` for every spatial extent, so the per-array shape check cannot see a file that came from a model built for another image size.
  - `load_from_path` refuses such a file and names both shapes. `load_partial_from_path` skips the layer whole, and every path of that layer goes into `missing` and into `unused` at the same time.
...
- **`Dense` accepts an input of rank 2 or more.** The layer contracts the last axis alone and replaces it with `output_dim`, so a `[N, T, input_dim]` input gives `[N, T, output_dim]`. 1 kernel serves every leading position, which is what lets 1 `Dense` layer transform every timestep of a sequence. Rank 3 is the common case.
  - The extra rank costs nothing. The leading axes fold into 1 row axis, so the pass stays the same single matrix product a rank-2 input runs, and the 2 forms agree bit for bit. The gradient goes back at the rank of the input, and `compute_output_shape` maps a rank-3 input to a rank-3 output. The layer builds for the canonical rank-2 form `(None, input_dim)`, so the summary line reports that form.
...
### Changed
- **Breaking change: 32 constructors lost the input shape they took, and every model-construction site must be rewritten.** Every convolution, `Flatten`, the 6 windowed pooling layers, `Dropout`, the 3 `SpatialDropout` layers, `GaussianNoise`, `GaussianDropout`, the 4 normalization layers, `PReLU`, `Dense`, `SimpleRNN`, `LSTM`, and `GRU` now take their own configuration alone. `Dense::new(units, activation)` replaces `Dense::new(input_dim, units, activation)`, and `Conv1D::new(filters, kernel_size, stride, activation)` replaces the form that also took an input shape. The shape reaches a layer through `Layer::build`, and `SequentialBuilder::build` calls that once per layer.
  - `Sequential::new`, `Sequential::new_with_seed`, `Sequential::add`, and the `Default` implementation are gone. `SequentialBuilder` replaces all 4, and it is the only way to reach a `Sequential`.
  - The 6 windowed pooling constructors and `Flatten::new()` are infallible and return `Self` rather than `Result<Self, Error>`. Nothing they still take can be invalid.
  - `Embedding::new(input_dim, output_dim)` keeps both arguments. Its `input_dim` is a vocabulary size and not a tensor axis, so it belongs to the configuration of the layer.
- **Behavior change: the 15 builders that drew a weight array now record a seed, and `build` spends it.** `with_random_state` on an unbuilt layer records the seed and draws nothing. On a layer that is already built it draws again from the new seed, so the order of the 2 calls no longer matters. A construction therefore takes nothing from the global seed stream, and a build takes 1 sub-seed per drawing layer, in stack order.
  - Splicing a seeded layer into a stack no longer moves the layers after it. The seeded layer resolves its own seed and leaves the global stream where it was, which is what the resolver always promised for an explicit seed.
  - The 6 builders that seed a dropout or noise mask are unchanged. A mask is not a weight, and it is drawn in the forward pass.
- **Behavior change: `Dropout`, `GaussianDropout`, and `GaussianNoise` accept an input of any shape.** Each one owns no array and reads no extent, so a build records the shape for `output_shape` and checks no later input against it. Keras enforces nothing for these 3 either. The empty-shape wildcard that the old constructors accepted is gone with the argument it belonged to.
- **Behavior change: a built layer refuses an input that disagrees with the shape it was built for.** The rank must match, and every axis after the batch axis that the build shape fixes must match as well. The batch axis is never checked, because 1 layer serves every batch size, and an axis the build shape leaves free is not checked either. A printed output shape is therefore a shape the model produces, which was not true before.
...
- **Behavior change: the rule that an input must not be shorter than the kernel now applies under `Valid` padding only, and it runs at the build.** A configuration that used to fail in the constructor is now built and refused. `Conv2D::new(1, (3, 3), (1, 1), Activation::Linear)` constructs, and its `build` for the shape `(1, 2, 2, 1)` returns `Error::InvalidInput`, which is where the old constructor error moved to. Under `Same`, and under `Causal` on a `Conv1D`, the same layer builds and runs and returns the size those 2 rules give. That is what Keras does. Measured across every convolution layer type and every padding mode.
  - The check cannot stay in a constructor. `with_padding` and `with_dilation_rate` both run after the constructor, and both decide whether the rule applies at all, so the padding mode is not final until the build.
  - The build is the right step for it, because the answer would otherwise be an axis of 0 positions. A 0 extent passes through every later layer of the stack and turns up far from the layer that made it, so `compute_output_shape` refuses instead of reporting one. The message names the layer, the axis, and the effective kernel extent that the axis must reach: ``Conv2D under Valid padding needs an input height of at least the effective kernel extent, which is 3. This input height is 2.`` Inside a model build the position of the layer is named as well.
  - All 7 layers that bound their input carry the rule: `Conv1D`, `Conv2D`, `Conv3D`, `DepthwiseConv1D`, `DepthwiseConv2D`, `SeparableConv1D`, and `SeparableConv2D`. Only `Valid` bounds the kernel by the input, because it reads complete windows alone. `Same` and `Causal` add the missing cells on the borders, so every extent stays legal there. The rule reads the effective kernel, so a dilated layer needs `(k - 1) * d + 1` cells and not `k`.
  - A transposed convolution grows its input and puts no such bound on it, so this rule never applied to those 3 layers. The 6 windowed pooling layers already refused a window larger than the input, and that refusal already ran at the build.
