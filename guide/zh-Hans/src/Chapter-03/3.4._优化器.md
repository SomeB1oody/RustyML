...
pub trait Optimizer {
    ...
    fn update(&mut self, scope: usize, layer: &mut dyn Layer, grad_scale: f32); // 每一层调用一次
...
训练循环在碰触任何一层之前，每个 batch 都恰好调用一次 `step`。之后，它对每一层调用一次 `update`，并把该层的下标作为 `scope` 传进去。有状态的优化器在 `step` 里推进自己的“时间”概念。Adam 和 AdamW 在这里递增用于偏差校正的时间步。SGD、RMSprop 和 AdaGrad 没有这样的计数器，所以 `step` 对它们什么也不做。`step` 不再倒回任何东西，因为已经没有任何优化器在走游标了。
...
`update` 会从训练循环收到 2 个值。第一个是 `scope`，也就是从输入端数起的层下标，最靠近输入的那一层的 `scope` 是 0。第二个是 `grad_scale` 系数，循环根据 `global_clipnorm`（见下文）算出它。不裁剪时，`grad_scale` 是 `1.0`。在 `update` 内部，优化器通过 `layer.parameters()` 向层索取参数。这个调用会为每个可训练张量返回一个扁平的 `ParamGrad`。`ParamGrad` 里有一个 `name`、一个可变的 `value` 切片、一个与之匹配的 `grad` 切片，以及一个 `decays: bool` 标志。
...
优化器按 `ParamId` 来索引自己的逐参数状态，这个 `ParamId` 把层的 `scope` 和层给张量取的 `name` 配成一对。这 2 半合起来给每个参数一个稳定的身份，于是优化器状态跟着张量走，而不是跟着它在列表里的位置走。这些名字就是 Keras 3 的名字：`kernel`、`recurrent_kernel`、`depthwise_kernel`、`pointwise_kernel`、`bias`、`embeddings`、`alpha`、`gamma` 和 `beta`。因此一个层交出张量的顺序是自由的，而且可以在各步之间变化。某个还没有梯度的张量只是缺席而已，它不会拖住同一层里的任何其他张量。

`Sequential` 会替你把层下标作为 `scope` 传进去。手写的循环必须在每一步都为同一个层传同一个 `scope`。一个优化器实例只属于一个模型，因为一个模型的 `scope` 0 并不是另一个模型的 `scope` 0。不要跨模型共享同一个优化器实例。
