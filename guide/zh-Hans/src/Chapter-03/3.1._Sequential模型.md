...
`Sequential` 是把一摞层组合成可训练模型的容器。搭建模型用的是另一个类型：按顺序把各层 `add()` 进一个 `SequentialBuilder`，再对着输入的形状 `build()`。这次调用交回一个 `Sequential`，你用一个优化器和一个损失函数 `compile()` 它，然后才调用 `fit()` 和 `predict()`。

RustyML 在 3 个地方与 Keras 不同：训练默认是全批量的；这里没有回调机制，提前停止和学习率调度都得自己写循环去实现；输入的形状只在 `build` 这 1 个地方交给模型，而不是靠一个 `Input` 层或者逐层的 `input_shape` 参数。
...
下面这 6 个调用，就是训练一个网络所需的全部公开接口。`add` 按值接收构建器再把它交回来，所以整摞层就是 1 个表达式。`build` 返回 `Result<Sequential, Error>`，而每一个训练调用都属于构建出来的那个模型。`fit` 返回 `Result<History, Error>`，每个 epoch 带一个损失值。`predict` 返回 `Result<Tensor, Error>`。
...
`add` 会消费掉这个层，所以你可以在一个表达式里现构造、现移动：`.add(Dense::new(8, Activation::ReLU).unwrap())`。它同样消费并交回构建器，所以整摞层都链在 1 个表达式里。层的构造函数本身也可能失败，比如激活函数带了一个不可用的参数就会返回 `Error::InvalidParameter`。这就是为什么 `.unwrap()` 挂在层的构造函数上，而不是 `add` 上。

**`add` 不做任何校验，校验全在 `build` 里。** `add` 不可能失败，因为在形状真正走过一个层之前，这个层没有任何地方能跟这摞层起冲突。`build` 从输入端把这摞层走一遍：它把到达每一层的形状交给那一层，再把每一层的输出形状串给下一层。某一层拒绝了自己拿到的形状，这趟遍历就停下，那个位置往后什么都不会分配。见 [3.1.8](#318-错误形状检查与构建期拒绝)。Keras 的函数式 API 也是这样：连接不兼容的层，会在搭建计算图时就失败。

这趟遍历也是每一个权重的来源。层的构造函数只接收这个层的配置，别的什么都不接收，因为 kernel 的尺寸来自输入，而输入此刻还不在。`Layer::build` 才是一个层认识那个形状并完成分配的地方。所以构建器手里一个权重都没有，而抵达 `Sequential` 的模型持有它需要的每一个权重。
...
pub trait Layer: std::any::Any + Send + Sync {
    ...
    fn build(&mut self, input: &Shape) -> Result<(), Error>;          // 依据输入形状分配数组
    fn compute_output_shape(&self, input: &Shape) -> Result<Shape, Error>; // 纯函数，不分配
    fn param_count(&self) -> ParamCounts;
    fn layer_type(&self) -> &str;
    // build()、compute_output_shape()、parameters()、output_shape() 有默认实现
}
```

`build` 和 `compute_output_shape` 这一对，是构建期遍历得以成立的原因。`build` 要分配数组，所以它拿 `&mut self`，每层只跑 1 次。`compute_output_shape` 是纯函数：它只读这个层的配置和输入形状，不碰任何前向传播写下的缓存，在一个从未构建、也从未运行过的层上也能给出答案。一个 `Shape` 每根轴 1 项，每一项要么是一个固定的尺寸，要么是一根自由轴。`summary` 打印的 `Layer::output_shape`，就是拿这个层持有的形状去跑 `compute_output_shape`。
...
RustyML 按类型生成层名，采用 Keras 风格：第一个 `Dense` 层叫 `dense`，下一个叫 `dense_1`，以此类推。输出形状里的 `None` 是一根自由轴，也就是用 `Shape::with_free_batch` 构建的模型的 batch 轴。同一摞层改用 `Shape::known(&[4, 2])` 构建，这 2 行就会打印成 `(4, 8)` 和 `(4, 2)`。

现在打印出来的形状，就是模型真正会产出的形状。`build` 已经把到达每一层的形状交给了那一层，所以每一行都来自整摞层已经彼此认可的形状。一个从未见过张量的模型，照样能为每个位置打印出真实的输出形状。`Sequential::input_shape()` 和 `Sequential::output_shape()` 报告的是这趟遍历的两端，而 `output_shape()` 同样不需要一次前向传播。

参数量来自每一层的 `param_count()` 方法。一个 `Dense(in -> out)` 层报告 `in * out + out` 个参数，也就是权重矩阵加偏置向量，所以 `2 * 8 + 8 = 24`，`8 * 2 + 2 = 18`。字节数按每个 `f32` 占 4 字节估算。

`param_count()` 返回一个 `ParamCounts`，里面有 `trainable` 和 `non_trainable` 两个字段。每一层一次报告两个计数，所以一个层整体上并不是“可训练”或“不可训练”的。`summary` 对全模型分别累加这两个字段：第一个字段打印成 "Trainable"，第二个打印成 "Non-trainable"，两者之和打印成 "Total"。无参数的层（比如激活函数和池化）在两个字段里都报告 0。`BatchNormalization` 在两个字段里都报告非零值：对 `C` 个通道，它持有 `2 * C` 个可训练元素，也就是 `gamma` 和 `beta`，以及 `2 * C` 个不可训练元素，也就是 `moving_mean` 和 `moving_variance`。因此它的合计是 `4 * C`，这 3 个数字与 Keras 报告的一致。这个计数是在默认设置下成立的：`with_center(false)` 去掉 `beta`，`with_scale(false)` 去掉 `gamma`，每个开关各从可训练计数里减去 `C`，不可训练计数则不受影响。`summary` 借的是 `&self`，从不改动模型，所以随时都能调用它，训练前训练后都行。
...
XOR 是线性模型解不了的最小问题，它是隐藏层确实在干活的经典证明。这个示例用了 2 个 `Dense` 层：一个 `Tanh` 隐藏层，加一个作用在 2 个类别上的 `Softmax` 输出头，用 Adam 和多分类交叉熵训练，就能把两类干净地分开。目标是 one-hot 的：类别 0 是 `[1, 0]`，类别 1 是 `[0, 1]`。用 `with_random_state(0)` 给两层的权重初始化播种，让这次运行可复现。构建器只记下这个种子，由 `build` 把它花掉，所以每一层只抽取 1 次自己的数组。
...
最后一种情况是这摞层的形状彼此不吻合，它也是变化最大的一种。**形状不匹配在 `build` 处就被拒绝，此时还没有任何数据流动，而且消息会点名那个层。** 没有哪一对相邻层能在沉默中彼此不吻合，因为层不再自行声明它期待的宽度。`build` 把到达每一层的形状交给那一层，而无法接受这个形状的层会让遍历停下。下面这个示例，让一个 4x4 的池化窗口去跑一张 3x3 的特征图：
...
这条消息读作 3 段。`layer 1` 是这摞层里的位置，从输入端数起。`MaxPooling2D` 是那个位置上的类型。`(None, 3, 3, 4)` 是真正到达的形状，由它前面那个卷积产出。这 3 段合起来，指向你模型里的 1 行代码。
