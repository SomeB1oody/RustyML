...
结果形状就是最后一层吐出的东西，二维输入配 `Dense` 收尾的话就是 `(batch, output_dim)`。输入必须符合每一层的预期：`Dense` 层要求一个秩为 2 或更高、且最后一根轴宽度为 `input_dim` 的张量，其他任何东西都会返回 `Error::InvalidInput`。
...
| 秩为 1 的输入喂给 `Dense` 层，或最后一根轴的宽度不是 `input_dim` | `Error::InvalidInput(_)` |
...
model
    ...
    .add(Dense::new(8, 2, Activation::Softmax { axis: -1 }).unwrap()); // 期望 8——不一致
