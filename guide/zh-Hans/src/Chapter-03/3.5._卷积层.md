...
`set_weights(...)` 装入你自己控制的权重和偏置。`set_weights` 会用层的期望形状校验每一个数组。注意深度卷积的 `set_weights` 接收一个 kernel 和一个 `Array1` 偏置。可分离卷积的则接收 3 个数组：depthwise 权重、pointwise 权重，然后是偏置。它们的偏置参数一律是可选的：对用 `with_use_bias(false)` 构造的层传 `None`，对其他层传数组。反过来传会返回 `Error::InvalidParameter`。
...
只有 `Valid` 这一种模式要求 kernel 装得进输入轴。`Same` 会把缺的格子补上，所以在它下面，有效 kernel 比输入轴还长也是合法的。因此这条规则在前向传播里执行，而不在构造函数里，因为 `with_padding` 和 `with_dilation_rate` 都在构造之后运行，而且都决定这条规则是否适用。一个 `3x3` kernel 配 `2x2` 输入的 `Conv2D` 在任何模式下都能构造成功。之后它在 `Valid` 下从 `forward` 返回 `Error::InvalidInput`，在 `Same` 下则正常运行。3.5.10 节把这两半都演示了一遍。
...
fn main() {
    ...
    let build = |dilation: usize, padding: ConvPadding| {
        let mut layer = Conv1D::new(1, 3, vec![1, 8, 1], 1, Activation::Linear)
            .unwrap()
    ...
    let out = build(2, ConvPadding::Valid).forward(&x).unwrap();
    assert_eq!(out.shape(), &[1, 4, 1]);
    // 位置 0 读到第 1、3、5 步：1 + 3 + 5 = 9。
    assert_eq!(out[[0, 0, 0]], 9.0);

    // Causal 填充保留长度，而且不读任何更靠后的步。
    let causal = build(1, ConvPadding::Causal).forward(&x).unwrap();
...
| `input_shape` 秩不对，或通道数为零（构造时） | `Error::InvalidInput` |
| `forward` 收到秩不对的张量 | `Error::InvalidInput` |
| 普通 `Valid` 卷积在运行时空间维小于有效 kernel | `Error::InvalidInput` |
...
这里有 2 条错误路径需要多说几句。第一条是 kernel 装不进自己那根输入轴。引擎只在前向传播里检查它，而且只在 `Valid` 填充下检查。构造函数无法检查它，因为 `with_padding` 和 `with_dilation_rate` 都在构造函数之后运行，而且都决定这条规则是否适用。在 `Valid` 下，引擎用 `usize` 计算 `in - keff`，与其让这个计算下溢，不如直接返回 `InvalidInput`。在 `Same` 下，以及 `Conv1D` 的 `Causal` 下，填充会把缺的格子补上，所以同一个层照样运行，并返回那两条规则给出的尺寸。
...
fn main() {
    // 1. kernel 比输入大的层能构造出来。只有 Valid 填充拒绝它，而且是在前向时。
    let mut small_input =
        Conv2D::new(1, (3, 3), vec![1, 2, 2, 1], (1, 1), Activation::Linear).unwrap();
    let tiny = Array::ones((1, 2, 2, 1)).into_dyn();
    assert!(matches!(
        small_input.forward(&tiny),
        Err(Error::InvalidInput(_))
    ));

    // 同一个层在 Same 填充下能跑通，因为填充把缺的格子补上了。
    let mut padded = Conv2D::new(1, (3, 3), vec![1, 2, 2, 1], (1, 1), Activation::Linear)
        .unwrap()
        .with_padding(PaddingType::Same);
    ...
    let bad_dilation = Conv2D::new(1, (3, 3), vec![1, 8, 8, 1], (2, 2), Activation::Linear)
        .unwrap()
        .with_dilation_rate((2, 2));
    assert!(matches!(bad_dilation, Err(Error::InvalidParameter { .. })));

    // 4. 运行时张量比 kernel 小：Valid 的几何计算返回错误，而不是 panic。
    ...
    // 5. DepthwiseConv2D 把运行时通道数不匹配转成可恢复的错误。
    let mut dw =
