...
这 2 个层服务于不同的方向。`Flatten` 是从卷积栈走向 `Dense` 头。`Reshape` 还能反着走，把 `Dense` 的输出重新变回一个立体的张量。解码器需要的正是这个方向。RustyML 目前还没有转置卷积，但 `UpSampling1D`、`UpSampling2D` 和 `UpSampling3D` 这几个层能把空间轴放大回去，正好用于这个方向。它们的形状和 `Interpolation` 模式见 [3.6. 池化层](./3.6._池化层.md)。
