...
本页依次介绍这些布局、构造函数与填充和膨胀规则、驱动标准卷积的 im2col + GEMM 引擎，深度可分离卷积的分解和它的开销计算，把卷积反着跑从而把张量放大的转置卷积，手动改动空间轴尺寸的边界层，重排轴或复制向量的那两个层，什么都不做的那个层，以及这些层会返回的错误类型。
...
`with_dilation_rate` 返回一个 `Result`。膨胀率为 `0` 时返回 `Error::InvalidParameter`。膨胀率大于 1 同时 stride 也大于 1 时返回同一个错误，因为这 2 个因子放在一起没有公认的含义。1D 的层接收标量，2D 和 3D 的层接收元组，和它们的 kernel 参数完全一致。全部 10 种卷积层都带这个方法。
...
| `with_dilation_rate` 收到 `0`，或膨胀率大于 1 的同时某个 stride 也大于 1 | `Error::InvalidParameter` |
