...
如果你用过 Keras，这里的布局和权重形状你已经很熟悉：张量是 channels-last，kernel 的抽头排在最前。构造函数只接收这个层的配置，别的什么都不接收。它从不接收输入形状，因为 kernel 的尺寸来自输入，而输入此刻还不在。`Layer::build` 读取形状、确定每个数组的尺寸，并抽取权重。模型的 build 对每一层调用它 1 次。
...
`with_dilation_rate` 返回一个 `Result`。膨胀率为 `0` 时返回 `Error::InvalidParameter`。膨胀率大于 1 同时 stride 也大于 1 时返回同一个错误，因为这 2 个因子放在一起没有公认的含义。Keras 也划了同样的界。1D 的层接收标量，2D 和 3D 的层接收元组，和它们的 kernel 参数完全一致。全部 10 种卷积层都带这个方法。
...
fn main() {
    ...
    let out = make(2, ConvPadding::Valid).forward(&x).unwrap();
    assert_eq!(out.shape(), &[1, 4, 1]);
    // 位置 0 读到第 1、3、5 步：1 + 3 + 5 = 9。
    assert_eq!(out[[0, 0, 0]], 9.0);

    // Causal 填充保留长度，而且不读任何更靠后的步。
    let causal = make(1, ConvPadding::Causal).forward(&x).unwrap();
...
fn main() {
    ...
    let message = match small_input.build(&Shape::with_free_batch(&[1, 2, 2, 1])) {
        Ok(()) => panic!("3x3 的 Valid kernel 装不进 2x2 的输入"),
    ...
    assert_eq!(padded.forward(&tiny).unwrap().shape(), &[1, 2, 2, 1]);
    ...
    assert_eq!(conv.forward(&batch_of_9).unwrap().shape(), &[9, 3, 3, 1]);
