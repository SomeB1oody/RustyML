...
因为这个乘积是每个块要做的工作量 —— 但它不是块的数量。折叠只切分样本轴，所以一个又宽又短的
输入可能靠特征数就越过了阈值，而折叠其实还是只有 1 个任务。这类调用点要给自己的乘积再配一道
下限，卡在折叠真正切分的那根轴上：至少 2 个 `DET_REDUCE_BLOCK` 块。阈值移动的是切换点，
但从不触碰正确性，因为分块折叠在切换点两侧给的是同一个答案。

每道阈值都配有一个对应的 setter，比如 `set_sum_f64` 或 `set_sq_sum_f32`，用来为不同硬件调整
切换点。[7.3. 性能调优与并行](../Chapter-07/7.3._性能调优与并行.md) 讲了具体机制和校准过程。

logistic 对数损失的归约也有一道阈值，是 32 768 个元素，但它是一个普通常量，不可调。那里每个
元素都要付一次 `exp` 和一次 `ln`，所以并行比普通加法更早摊平成本。这个值由分块大小定死：折叠
把范围切成 `DET_REDUCE_BLOCK` 大小的块，所以低于 2 块就只有 1 个任务，根本谈不上并行。
