...
RustyML 把它们委托给 [`gemmkit`](https://crates.io/crates/gemmkit)，一个纯 Rust 的 GEMM 引擎。crate 经由它那层零拷贝的 [`gemmkit-ndarray`](https://crates.io/crates/gemmkit-ndarray) 适配器够到 gemmkit。`math` feature 会引入这两个 crate。RustyML 自己只在 `src/math/matmul.rs` 里留下薄薄一层代码。
...
前 2 个条目对 `T: gemmkit::GemmScalar` 泛化。在 RustyML 的构建里，这恰好就是 `f32` 和 `f64`。gemmkit 另外还能在可选的 `half` feature 下支持 `f16` 和 `bf16`，在 `int8` feature 下支持 `i8`。RustyML 两个都没打开，所以这里既没有半精度，也没有整数 matmul。
...
它在一个字节下限之下保持串行，也就是 `gemv_parallel_bytes`，默认是 `0`（意思是"按末级缓存的大小推导"）。低于这个下限时，被触及的数据是 LLC 常驻的。1 个核就已经吃满了全部 LLC 带宽，所以再切分只会白白添上 fork-join 开销和缓存争用。

越过这个下限之后，工作会切分到一个按内存带宽设定的 worker 上限之内，也就是 `gemv_thread_cap`，同样默认是 `0` 表示自动。DRAM 饱和所需的 worker 数，远少于机器的逻辑核数。第三个旋钮 `gemv_threshold`，限定向量那一侧最大能到多少，超过就把这个形状退回通用驱动。它的默认值是 `usize::MAX - 1`，实际上等于无上限。所以在实践中，一个 gemv 形状的问题总会走 gemv 路径，除非你自己调低这个旋钮。
...
真正的 matvec 从不出现在里面，因为一次 `Dense` 前向永远不是 matvec。matvec 会彻底离开通用驱动，转投 gemmkit 的 gemv 路径。它改用一个从末级缓存推导出来的字节下限来卡，随后把 worker 数限制在内存系统真正喂得动的范围内。这条路径受限于带宽，多加的核在那里开始划算的时机，远早于一个受算力限制的 GEMM 能摊平线程派发开销的时候。
