...
前 2 个条目对 `T: gemmkit_ndarray::GemmScalar` 泛化。在 RustyML 的构建里，这恰好就是 `f32` 和 `f64`。gemmkit 另外还能在可选的 `half` feature 下支持 `f16` 和 `bf16`，在 `int8` feature 下支持 `i8`。RustyML 2 个都没打开，所以这里既没有半精度，也没有整数 matmul。

RustyML 唯一打开的非默认 feature 是 `epilogue`，开在 `gemmkit-ndarray` 上，为的是那条融合路径。如果你需要 `f16`、`bf16` 或 `i8` 支持，就得自己直接对着 gemmkit 写代码。

后 2 个条目根本不是乘积。它们是调用侧的分块策略，供那些成对投影大到一次装不下、否则就得整块物化的估计器使用。这 4 个条目都是 crate 内部的，从外部一个也调不到。改用 [6.2.5](#625-调整阈值公开接口) 里那些管着这套策略的旋钮。

下面是各个部分分别在哪里被调用：

- `Dense::forward` 是一次 `gemm_fused` 调用。它把线性乘积、按列的偏置，以及 ReLU 激活，融进了 1 趟里。
- `Dense::backward` 是 2 次普通的 `dot` 调用。第一次算权重梯度。第二次算输入梯度。
- `SimpleRNN`、`LSTM` 和 `GRU` 用 `dot` 把输入一次性投影好。随后每个时间步都用 `gemm_fused` 融合它的递归投影。算递归核的梯度时，3 者都降级到普通的 `gemm`，因为那要写进一个更大缓冲区的某个列块。
...
- KNN、t-SNE 和 MeanShift 用 `cache_resident` 和 `gemm_chunk_rows`。这 2 个函数在"逐行 GEMV swarm"与"分块 GEMM"之间为一次成对投影做选择。
...
一个 gemmkit 旋钮按优先级依次定值。单次调用的实参，比如 `Parallelism` 请求，压过程序里的 `set_*` 调用。`set_*` 调用压过 `GEMMKIT_*` 环境变量。环境变量压过编译期默认值。gemmkit 只在该旋钮首次被访问时读一次每个环境变量，此后整个进程都用缓存值。
...
gemmkit 只建一次这些分档池，然后热着复用，不会每次调用都重建。设成 `0` 就完全禁用它们。默认值按架构分裂：x86-64 上 2 档，aarch64 上 1 档，其余每个目标上 0 档，等着在设备上验证。
...
| `get_cache_resident_max_bytes` / `set_cache_resident_max_bytes` | 33,554,432 | 常驻缓存的尺寸阈值，设为单个核心能直接够到的那部分 L3 |
| `matmul::backend::*` | 见 gemmkit | 后端的每一个旋钮，各自对应一个 `GEMMKIT_*` 环境变量 |

`cache_resident_max_bytes` 是你最可能想改动的那个旋钮。把它设为你机器上**单个核心能直接够到**的那部分 L3，绝不要设成整颗 CPU 的总量。每一趟扫描都跑在自己的 rayon 任务里，所以真正起作用的是跑它那个核心本地的那一片；在多 die 的处理器上，另一个 die 的那一片要走 fabric 往返，代价接近 DRAM。默认值 32 MiB 是标定机器的单 die L3 —— 它那 64 MiB 分成 2 份。这个值周围那一带没有标定过，而且这道门只算了共享矩阵，没算分块分支要分配的那块缓冲区。
...
这些阈值把"多线程在哪里有用"编码了进去。一次 `Dense` 前向就是 1 次融合的后端 GEMM。偏置和激活都跑在内核的尾声里，不是额外的趟数。训练上下文还会为反向传播存放一份输入缓存和一份输出缓存，所以要单独给这次投影计时，请用推理上下文。
...
- 深 `k` 的形状是有意思的那个。这里 `m` 和 `n` 都很小，而 `k` 在增长。一种常见的直觉认为凡是细长的形状就一定受带宽限制。深 `k` 的乘积则通常反而受算力限制，因为相对它所喂进去的算术量而言，操作数很小。它是深度分块决策最要紧的形状。
...
你可以在本机上、围着一个固定的乘积来回拨动闸门，快速比一比串行与并行。下面的例子通过一个公开的 `Dense` 层驱动这套后端，把同一个乘积 2 种方式各计时一遍。把打印出的数字当草图看，不要当基准，因为单次调用噪声很大。要真实数据，就用会预热、会重复的 `cargo bench --bench nn_end_to_end`。
...
fn main() {
    ...
    // `build` 分配核并花掉记录下来的种子。`forward` 接收 `&self`，
    // 因此它无法构建，也会拒绝一个未构建的层。
