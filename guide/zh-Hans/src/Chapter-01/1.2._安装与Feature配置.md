...
ndarray = "0.17"
```
- 第一行依赖的意思是使用0.16版本的`rustyml` crate，使用默认feature(也就是开启全部的feature)，这样一来RustyML所有的功能你都可以使用了。
...
RustyML使用`ndarray`提供的数组类型，不论输入还是输出。经典机器学习模块接收`Array2<f64>`特征矩阵和`Array1<f64>`目标向量，神经网络则使用`Tensor`，也就是`ArrayD<f32>`别名（注意机器学习和工具模块用 `f64`，神经网络用`f32`）。RustyML 0.16基于`ndarray` 0.17构建，确保你的`Cargo.toml`文件里`ndarray`的版本号是`0.17`。
