...
构造器只接收该层的配置。`build`把输入的`[batch, length, channels]`形状交给该层。`compute_output_shape`在任何张量出现之前就能给出输出形状。直接调用`forward`会用收到的张量来构建该层。已构建的层随后会拒绝阶或通道数对不上的张量，并接受任何批大小。逐层的细节见[3.5 卷积层](../Chapter-03/3.5._卷积层.md)与[3.6 池化层](../Chapter-03/3.6._池化层.md)。
