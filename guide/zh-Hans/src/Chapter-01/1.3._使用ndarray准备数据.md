...
RustyML的所有矩阵类型全部使用[`ndarray`](https://docs.rs/ndarray/0.17/ndarray/)提供的类型。如果你想要更加详细地了解ndarray的进阶用法，比如广播规则、各类迭代器、线性代数辅助函数，可以去看[官方ndarray文档](https://docs.rs/ndarray/0.17/ndarray/)。

RustyML 0.16依赖ndarray 0.17。在你自己的`Cargo.toml`里需要使用相同的ndarray版本：
...
很多时候我们希望**随机**初始化矩阵内容。这时候[`ndarray-rand` crate](https://crates.io/crates/ndarray-rand)提供了`Array::random`，你需要在`Cargo.toml`里面把它加入依赖项：
...
搭配RustyML使用时不要手动去给`ndarray-rand`的随机数生成器播种。RustyML有自己的随机性管理系统(`set_global_seed`、每个模型上的`with_random_state`，以及`SequentialBuilder::new_with_seed`)，你可以通过RustyML提供的API来控制随机性，详见[7.1 可复现性与随机种子](../Chapter-07/7.1._可复现性与随机种子.md)。
...
有2处与它不同，在[1.3.1](#131-f64与f32)的表格里都能看到：`LDA`的`y`是`Data<Elem = i32>`而不是`f64`；无监督的估计器（`KMeans`、`DBSCAN`、`MeanShift`、`PCA`、`KernelPCA`）只接收特征矩阵，根本没有`y`这个参数。

ndarray中有2种引用类型：

- 拥有型数组的引用(`&Array`)
- 视图(`ArrayView`)，也就是`.view()`和`.slice(..)`产出的东西
...
| `MaxPooling{1,2,3}D`、`AveragePooling{1,2,3}D`                      | `[batch, spatial..., C]`             | 无                                                                  | 无          | `[batch, pooled..., C]`                 |
| `GlobalMaxPooling{1,2,3}D`、`GlobalAveragePooling{1,2,3}D`          | `[batch, spatial..., C]`             | 无                                                                  | 无          | `[batch, C]`                            |
| `SpatialDropout{1,2,3}D`                                            | `[batch, spatial..., C]`             | 无                                                                  | 无          | 与输入同形                              |
| `BatchNormalization`、`GroupNormalization`、`InstanceNormalization` | `[batch, spatial..., C]`             | `gamma`/`beta` `[C]`                                               | 无          | 与输入同形                              |
| `PReLU`                                                             | `[batch, d1...]`                     | `alpha [d1...]`，每个共享轴上为 `1`                                | 无          | 与输入同形                              |
...
fn main() {
    // filters=2, kernel=3, stride=1。这里没有形状：该层从`build`取得形状
...
构造器只接收该层的配置。`build`把输入的`[batch, length, channels]`形状交给该层。`compute_output_shape`在任何张量出现之前就能给出输出形状。直接调用`forward_mut`会用收到的张量来构建该层。已构建的层随后会拒绝阶或通道数对不上的张量，并接受任何批大小。逐层的细节见[3.5 卷积层](../Chapter-03/3.5._卷积层.md)与[3.6 池化层](../Chapter-03/3.6._池化层.md)。
...
有2套不同的错误系统在抓形状问题。发生在你**构造**数组阶段的错误来自ndarray，比如当缓冲区长度和形状对不上时，`from_shape_vec`返回ndarray的`ShapeError`。发生在**RustyML**运行期间的错误会返回RustyML自己的[`Error`](../Chapter-01/1.6._错误处理.md)枚举，它还会对错误进行进一步细分：
- `Error::DimensionMismatch { expected, found }`：2个标量不一致，比如`predict`时的特征数与`fit`时不一致，又比如`x.nrows()`与`y.len()`不一致
- `Error::ShapeMismatch { expected, found }`：2个完整的张量形状对不上，比如神经网络栈里某个梯度和它要流入的激活形状不符
