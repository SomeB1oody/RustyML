...
`NotBuilt`是这 5 个里最新的一个。层在`Layer::build`里分配它持有的每个数组，所以从未构建过的层既没有核也没有偏置。`Layer::predict`接收`&self`，无法构建，因此它报这个变体。经由`SequentialBuilder::build`得到的模型总是已构建的，永远不会抛出它。
