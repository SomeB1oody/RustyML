...
RustyML只有 1 种错误类型`rustyml::error::Error`。RustyML里每一个可能失败的操作都返回`Result<T, rustyml::error::Error>`，它还有一个别名：
...
| `DimensionMismatch { expected, found }` | 2 个标量计数对不上                                    | `dimension mismatch: expected <e>, found <f>`                            |
| `ShapeMismatch { expected, found }`     | 2 个张量的形状对不上（梯度与它流入的那个激活值）      | `shape mismatch: expected [..], found [..]`                              |
| `NonFinite(String)`                     | 数据中的某个值为 `NaN` / `inf`，或者计算产出了这样的值 | `non-finite value (NaN or infinity) encountered in <where>`              |
...
`Error`的 3 个变体各自包装了一个更小的枚举。只与神经网络相关的问题（层状态、权重形状、编译）和只与树相关的问题（分类还是回归）都待在各自的枚举里。
...
`NotBuilt`是这 5 个里最新的一个。层在`UnaryLayer::build`里分配它持有的每个数组，所以从未构建过的层既没有核也没有偏置。`UnaryLayer::forward`接收`&self`，无法构建，因此它报这个变体。`UnaryLayer::forward_mut`接收`&mut self`，会用张量把层构建起来，因此它永远不抛出这个变体。经由`SequentialBuilder::build`得到的模型总是已构建的，永远不会抛出它。
...
**`TreeError`**（位于`rustyml::machine_learning::TreeError`）有以下 2 个变体:
...
**`IoError`**（位于 `rustyml::error::IoError`）有 4 个变体：
...
RustyML整个库只使用1种错误类型，所以一整个管线上的错误都可以作为`Error`返回，除了`Result`和`?`之外什么都不需要：
...
当你确实需要汇报外部错误（来自标准库或别的 crate），但是又想使用进这套错误处理体系、同时保留它的成因链时，就用`Context`扩展trait（需要把这个trait导入到作用域）。RustyML为任何满足`Send + Sync + 'static`且实现了`std::error::Error`的`Result<T, E>`都实现了它，因此能和`?`配合。`context`会立即取用信息，`with_context`接收一个只在错误路径上运行的闭包，只要构造信息会带来分配（凡是用到 `format!` 的），就优先用闭包形式，这样成功路径就不用执行闭包：
