...
归一化层在 `tuning::norm::` 下又添了 7 道门：`set_/get_batch_norm`、`set_/get_bn_col_stats`、`set_/get_bn_plane_stats`、`set_/get_ln_row`、`set_/get_ln_col_stats`、`set_/get_gn_row`、`set_/get_gn_param_grad`，这 7 道全都是 262_144。聚类指标再添一道 `tuning::metrics::set_/get_silhouette`，同样是 262_144。

某道门对应的 feature 没编进来，这道门就干脆不存在：`tuning::conv`、`tuning::pool` 和 `tuning::norm` 需要 `neural_network`，`tuning::tree` 需要 `machine_learning`，`tuning::metrics` 需要 `metrics`，`tuning::matmul` 和 `reduction::exp_reduce` 需要 `math`。
