...
越过这道门之后，worker 数是随总工作量、而不是随某一个维度往上爬的：gemmkit 每 `par_mnk_per_worker`（默认 2,000,000）份 `m * n * k` 配一个 worker，下限为 1，上限由核心数和任务数共同封顶。具体的临界点因机器而异：可以通过 `tuning::matmul::backend` 查看当前值，也可以用 `gemmkit-tune` 自动调参器在你自己的机器上实测（见 [7.3.5](#735-重新校准两套工具然后才是-setter)）。
...
矩阵乘积从前给的承诺弱一些：老的手写切行会让块高随线程数变化，从而改变求和顺序。现在不会了。gemmkit 的分块和任务顺序都与 worker 数无关，所以在同一台机器、同一套配置下，同一个乘积无论由多少线程跑出来，都复现出*逐位相同*的结果，重复运行也是确定的。
...
这里大多数默认值都是在维护者的硬件上量出来的。[`tuning`](https://docs.rs/rustyml/latest/rustyml/tuning/index.html) 模块文档记录了这台机器：一颗 AMD Ryzen 9 9950X，16 核、32 线程、64 MiB L3。没在那上面量过的少数几个只会更糟，不会更好：`cache_resident_max_bytes` 是一个有依据的猜测，`pool_parallel_min_ops` 则是被刻意停在它实测区间之外的，原因见 [7.3.5](#735-重新校准两套工具然后才是-setter)。
...
这是两台真实的机器，不是你那台机器的模型。凡是临界点依赖架构的旋钮，都带一个按 `cfg(target_arch)` 分支的默认值，每一支在各自的参考机器上标定；在任何第三种架构上，池档位默认关闭，等着在真机上验证。用 `gemmkit-tune` 自动调参器在你自己的机器上实测（见 [7.3.5](#735-重新校准两套工具然后才是-setter)），再把结果套成一份 `GEMMKIT_*` 配置。
...
这张表也说明了为什么值得重新跑一遍校准。`CONV_PARALLEL_MIN_FLOPS` 出厂值是 4,000,000，早先那一次测量把它放在区间之内。后来串行卷积路径快了一倍多，而这会把交叉点往右推，于是这个常量现在落在区间之下，而不再是区间之内。一道闸门标定的，是测量当天那个版本的代码。

按那行 takeaway 的方式去读这张表：门的常量应该落在临界点上，并留一点朝串行一侧的安全余量。为了在中等规模上多赚 5%，却在小张量上付出 2 倍的代价，是笔糟糕的买卖，而这道梯子把这种不对称摆到了明面上：在 84,672 FLOP 处，并行路径的速度只有串行的*一半*；在 830 万 FLOP 处，它也只快了 1.29 倍。
...
串行池化的速度强烈依赖通道数：窗口几何形状的开销可以摊到各个通道上。所以 25,088 抽头的 `1x28x28x32` 那一级串行只要 14.8 us，而*更小*的 16,384 抽头的 `64x16x16x1` 那一级却要 77.7 us。光看抽头数预测不了串行代价。这也正是结论行要把工作量相同的几级塌缩到最慢那一级的原因。

`POOL_PARALLEL_MIN_OPS` 就是那个现成的例子。它停在 12,000，刚好落在基准现在给它报出的区间下沿，两者是一致的。但过去并非如此：早先的结论规则盯的是*最后*一级亏损，于是把报出的区间推过了 16,384 抽头那一级的实测胜出，把它藏了起来。**校准输出是证据，不是指令。**在按某个区间去挪一个常量之前，先看清你的负载究竟落在这道梯子的哪几级上。
