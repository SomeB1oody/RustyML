...
RustyML 的并行只依赖 [rayon](https://docs.rs/rayon)，不依赖别的：既不链接 BLAS，也不用 OpenMP。crate 里的每个内核都跑在 rayon 的全局线程池上，只有一个例外。

这个例外就是矩阵乘积后端 [gemmkit](https://docs.rs/gemmkit)。gemmkit 常驻着少数几个私有 rayon 线程池，每个的宽度都是机器宽度的某个分数。它会装上仍然装得下这次乘积所需全部 worker 的那个最小档位，好让 fork-join 步骤看不到任何空闲余量。
...
每个并行内核背后都守着一道*阈值门*：把一份工作量估计拿去和校准好的阈值比较。小输入始终留在单核上跑。[`tuning`](https://docs.rs/rustyml/latest/rustyml/tuning/index.html) 模块在运行期挪动这些阈值门。随包发布的默认值可能不适合你的 CPU。不靠猜就能重新校准它们。
...
rayon 不是免费的。把一个任务交给线程池，要付出一次 fork、一次 join，归约还要搭上一些收集的开销。对一次大矩阵乘积，或者上百万元素的 `exp` 映射，这点开销淹没在计算量里根本看不见。但对 1 个 LSTM 时间步里的 32x32 GEMM，或者几千个激活值上的 ReLU，fork/join 步骤本身*就是*全部运行时间，这时并行版反而输给一个跑在内存带宽上的单核。

所以每个带门的内核都先估计自己的工作量：卷积算 FLOP，映射和归约数元素个数，树遍历数节点访问次数，池化数窗口抽头（window tap）数。只有这个估计越过按临界点校准的阈值，内核才会去用 rayon。

矩阵乘积本身，RustyML 并不自己把门：那里的调度完全属于 [gemmkit](https://docs.rs/gemmkit)。[6.2. 矩阵乘法](../Chapter-06/6.2._矩阵乘法.md) 讲了这个后端，以及本 crate 给它添的那一点点东西。gemmkit 决策的形状和 RustyML 自己那些门不一样。
...
越过这道门之后，worker 数是随总工作量、而不是随某一个维度往上爬的：gemmkit 每 `par_mnk_per_worker`（默认 2,000,000）份 `m * n * k` 配一个 worker，下限为 1，上限由核心数和任务数共同封顶。具体的临界点因机器而异：可以通过 `tuning::matmul::backend` 查看当前值，也可以用 `gemmkit-tune` 自动调参器在你自己的机器上实测（见 [7.3.5](#735-重新校准2-套工具然后才是-setter)）。

爬出来的这个宽度，随后会被吸附到前面说的某个常驻池档位上。rayon 的 fork/join 税是随池子的*空转余量*——池宽减去真正在干活的 worker 数——增长的，所以一个 8 worker 的乘积跑在恰好 8 宽的池子里，会大幅胜过同一个乘积跑在 32 宽的全局池里。

matvec 形状（`m == 1` 或 `n == 1`）会离开这条路径，走一条专门的、受带宽限制的路：低于一个由机器 L2 缓存大小推算出来的字节下限时保持串行，越过之后再按一个按内存带宽定的 worker 上限切分。

因为 RustyML 自己的内核跑的都是*全局* rayon 线程池，嵌套时它们能安全组合。一个并行的外层循环调进一道带门的卷积或归约，不会让机器超额订阅：内层工作会嵌进同一个池子，而不是再掀起一波线程。
...
下面这条来自 [`tuning`](https://docs.rs/rustyml/latest/rustyml/tuning/index.html) 模块文档的契约，让调参变得安全：
...
这一段话里藏着两条不同的保证。elementwise 映射（ReLU、sigmoid、缩放、归一化）是易并行的：每个输出元素彼此独立，串行和并行给出*逐位相同*的结果，挪动门连一个 bit 都动不了。

归约（平方和、Welford 矩、按全局范数裁剪）就微妙一些：浮点加法不满足结合律，朴素的 rayon `sum` 会按 work-stealing 的方式给部分和分组，每次运行的舍入都不一样。RustyML 用 [`crate::math::reduction`](../Chapter-06/6.3._并行归约.md) 里的确定性分块折叠绕开了这个问题：输入被切成固定大小的块，分组只取决于编译期的块大小，从不取决于线程数或阈值门。

于是归约门以上，并行路径依然与串行结果吻合，挪动门也永远不会改变它。

gemmkit 的分块和任务顺序都与 worker 数无关，所以在同一台机器、同一套配置下，同一个乘积无论由多少线程跑出来，都复现出*逐位相同*的结果，重复运行也是确定的。
...
[6.3](../Chapter-06/6.3._并行归约.md) 里那些确定性归约在种类上依旧是更强的保证：它们是靠构造给出同样的答案，不依赖后端在两台机器上恰好按同样的方式分块。

落到实处的结论没变：**你可以随意重调任何一道门，模型的输出都不会挪动。**门是性能旋钮，绝不是数值旋钮。如果你追求的是跨机器的精确可复现，那属于 [7.1. 可复现性与随机种子](./7.1._可复现性与随机种子.md) 的范畴，不在这里。

## 7.3.3. tuning 模块：运行期覆盖入口

每道门都是一个进程全局的 `AtomicUsize`，初始化为它校准好的默认值。[`tuning`](https://docs.rs/rustyml/latest/rustyml/tuning/index.html) 模块是让每道门都便于查阅的门面：它为每道门暴露一个 `set_*(usize)` 函数和一个 `get_*() -> usize` 函数，按内核族分进各个子模块。setter 和 getter 都是朴素函数。像这样就能读出你这份构建随包附带的默认值：
...
| `set_/get_cache_resident_max_bytes` | 33_554_432 | 单个核心能直接够到的 L3 字节数，用于"逐行 GEMV 群"与"分块 GEMM"之间的取舍 |
...
| `set_/get_exp_map_f32` | 131_072 | f32 以 exp 为主的映射：sigmoid、tanh、softmax、ELU、SELU、softplus、exponential 的**前向**。大部分对应的反向传播复用缓存的激活值，不调用 `exp`，因此归在 cheap-map 那一类 |
...
| `tuning::conv::` / `tuning::pool::` / `tuning::upsampling::` | 默认值 | 门限依据 |
| --- | --- | --- |
| `conv::set_/get_parallel_min_flops` | 4_000_000 | im2col+GEMM 卷积引擎与转置卷积引擎，估计 FLOP |
...
| `upsampling::set_/get_parallel_min_ops` | 2_000_000 | 上采样引擎，目标元素数乘以抽头数 |

归一化层在 `tuning::norm::` 下又添了 3 道门：`set_/get_batch_norm`、`set_/get_col_fold` 和 `set_/get_row_pass`。这 3 道都出厂在 262_144。在 BatchNorm 那道总闸门以下，3 个归一化层其实只有 2 种内核形状：按通道的列折叠，和按行的扫描，所以各用 1 道门就够了。`col_fold` 管 BatchNorm 的按通道统计量，以及 LayerNorm 和 GroupNorm 的 gamma/beta 梯度；`row_pass` 管 LayerNorm 的行扫描，以及 GroupNorm 的逐样本扫描。

有 2 道门是刻意不放进这套接口的。logistic 损失的 exp 归约是个普通常量，因为分块大小已经把它定死了。轮廓系数那道门同样是普通常量，而且它是唯一一道必须如此的：它的并行填充在数值上与串行一致，但并非逐位一致，所以在那里放一个公开 setter 会改变返回的分数，破坏上面那条承诺。

某道门对应的 feature 没编进来，这道门就干脆不存在：`tuning::conv`、`tuning::pool` 和 `tuning::norm` 需要 `neural_network`，`tuning::tree` 需要 `machine_learning`，`tuning::matmul` 需要 `math`。

`elementwise` 那几道门分成两半：f32 那一半归 `neural_network`，f64 那一半归 `machine_learning` 或 `utils`。只编译你用得上的模块（见 [1.2. 安装与Feature配置](../Chapter-01/1.2._安装与Feature配置.md) 和 [7.4. 按需裁剪与模块化集成](./7.4._按需裁剪与模块化集成.md)），无关的旋钮自然就消失了。

这张清单之所以短，是有意为之：RustyML 拥有两个 matmul 旋钮和二十来道内核门，每一道都是一个工作量估计上的阈值。

这里没有按 dtype 分的矩阵乘积门。f32 和 f64 的 GEMM 共用 1 个临界点，因为后端是按 `m * n * k` 把门的。元素位宽是后端的事，不是你的事。如果某个矩阵乘积在你的机器上跑出了不对的并行度，去 `backend` 里找，不在这张表里。

## 7.3.4. 为什么随包默认值可能不适合你的机器

这里大多数默认值都是在维护者的硬件上量出来的。[`tuning`](https://docs.rs/rustyml/latest/rustyml/tuning/index.html) 模块文档记录了这台机器：一颗 AMD Ryzen 9 9950X，16 核、32 线程、64 MiB L3。没在那上面量过的少数几个只会更糟，不会更好：`cache_resident_max_bytes` 是一个有依据的猜测，`pool_parallel_min_ops` 则是被刻意停在它实测区间之外的，原因见 [7.3.5](#735-重新校准2-套工具然后才是-setter)。
...
最明显绑死在特定机器上的一道门是 `cache_resident_max_bytes`。把它设成单个核心能直接够到的那部分 L3，绝不要设成整颗 CPU 的总量：每一趟扫描都跑在自己的 rayon 任务里，而在多 die 的处理器上，另一个 die 的那一片要走 fabric 往返，代价接近 DRAM。默认值 32 MiB 是标定机器的单 die 数字 —— 它那 64 MiB 分成 2 份。这个值周围那一带仍未标定。
...
这是两台真实的机器，不是你那台机器的模型。凡是临界点依赖架构的旋钮，都带一个按 `cfg(target_arch)` 分支的默认值，每一支在各自的参考机器上标定；在任何第三种架构上，池档位默认关闭，等着在真机上验证。用 `gemmkit-tune` 自动调参器在你自己的机器上实测（见 [7.3.5](#735-重新校准2-套工具然后才是-setter)），再把结果套成一份 `GEMMKIT_*` 配置。
...
这个扫参工具在它所针对的机器上运行，吐出一份可以直接 `source` 的 `GEMMKIT_*` 环境变量配置。这就是部署路径，而且是条好路径：环境变量能给一个已经编译好的二进制重新调参，不必重新编译，于是同一个产物可以给每台主机带上不同的配置。
...
# 打印这些表，并写入 target/parallel_gates/RESULTS.md。
# 设 RUSTYML_WRITE_RESULTS=1 可改为刷新纳入版本管理的 benches/calibrations/RESULTS.md。
cargo bench --bench parallel_gates
```

它需要 `machine_learning` 和 `neural_network` 两个 feature，子模块覆盖卷积引擎、池化、elementwise 与归约内核、树遍历，以及归一化层。对每一类内核，它把串行实现和并行实现在该类那道门的两侧各自强制跑一遍，走完一整梯子的形状，逐级报出加速比。

下面是随包 `benches/calibrations/RESULTS.md` 里的一段真实摘录，2026-08-21 在那台 32 rayon 线程的 9950X 上重新生成：
...
这张表也说明了为什么值得重新跑一遍校准。`CONV_PARALLEL_MIN_FLOPS` 出厂值是 4,000,000，落在实测区间之下。代码的一次改动可能改变串行代价或并行代价，从而挪动交叉点，而出厂的这个常量不会自己跟着挪。一道闸门标定的，是测量当天那个版本的代码。

按那行 takeaway 的方式去读这张表：门的常量应该落在临界点上，并留一点朝串行一侧的安全余量。在小张量上付出很大的代价，只为在中等规模上赚一点点，是笔糟糕的买卖，而这道梯子把这种不对称摆到了明面上。在 84,672 FLOP 处，并行路径的速度不到串行的一半（0.40x）。在 8,294,400 FLOP 处，也就是临界点之下不远，它依然落后于串行（0.85x）。直到越过临界点，在 35,426,304 FLOP 处，并行才反超，达到 1.51x。

这里有个陷阱。基准测试对每一级只报一个工作量数字，但一个内核的串行代价并不总是这个数字的函数。

结论那一行按 2 条规则读这道梯子。工作量相同的几级会塌缩到其中最慢的一级，因为一个交叉点的断言必须对所有达到该工作量的形状都成立。在交叉点之上，允许有 1 级回落成亏损而不推翻这个交叉点，结论行会把那一级点出来。第 2 条规则很要紧：当最大的那一级工作集超出缓存时，它会掉到内存带宽上限，于是不论多少线程都只有大约 1.00x。这样一级要读成关于带宽的结论，绝不是关于调度的结论。

串行池化的速度强烈依赖通道数：窗口几何形状的开销可以摊到各个通道上。所以 25,088 抽头的 `1x28x28x32` 那一级串行只要 15.1 us，而*更小*的 16,384 抽头的 `64x16x16x1` 那一级却要 80.4 us。光看抽头数预测不了串行代价。这也正是结论行要把工作量相同的几级塌缩到最慢那一级的原因。

`POOL_PARALLEL_MIN_OPS` 就是那个现成的例子。它停在 12,000，刚好落在基准现在给它报出的区间下沿，两者是一致的。一个单独命名的临界点，不总能说清一道有不止 1 处局部胜出的梯子，结论行只是个摘要，不是全部证据。**校准输出是证据，不是指令。**在按某个区间去挪一个常量之前，先看清你的负载究竟落在这道梯子的哪几级上。
...
fn main() {
    ...
    tuning::matmul::set_cache_resident_max_bytes(16 * 1024 * 1024); // 单 die 16 MiB 的 L3
...
如果你想在动手之前，先看看并行在你的机器上究竟哪里划算，跑一遍 `cargo bench --bench nn_end_to_end` 就知道了。

## 7.3.6. 掌控 rayon 本身

阈值门决定*要不要*并行，rayon 决定*铺多宽*。RustyML 自己的内核用的是 rayon 的全局池，从不自建，所以 rayon 标准的那套控制手段对它们原样适用，例外是后端的私有档位池，往下数两段就会讲到。
...
这里有一个相互作用最容易让人猝不及防。缩小环境线程池*不会*让矩阵乘积跟着缩放。gemmkit 是从工作量算出自己的 worker 数的——`m * n * k` 除以 `par_mnk_per_worker`——再拿机器的核心数封顶；这个核心数它只从 `std::thread::available_parallelism()` 读一次，然后按进程缓存下来，从不去问 `rayon::current_num_threads()`。
...
危险在于在那个池*之外*再造出*两个*并行来源：一个是 OS 线程池，一个是 `std::thread::spawn` 的扇出，还有一个是用 `ThreadPoolBuilder::build()` 而非 `build_global` 建出来的第二个 rayon 池。不管哪种情况，每个 worker 都各自调进 rustyml。此时每个 worker 的带门内核都各自去填满那个全局池，你就得到 `threads * threads` 的争用。
...
这些门是最后才该动的东西，不是第一个。碰它们之前，先端到端地测：给你真正在意的那个 `fit`、`predict` 或训练循环计时，找出时间花在哪。
...
第三，如果你依赖分块乘积那条路径（KNN、t-SNE、MeanShift），把 `cache_resident_max_bytes` 设成你 CPU 上单个核心能直接够到的那部分 L3。从 `lscpu` 上读：把 L3 总量除以实例数。
