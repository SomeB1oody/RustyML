...
| --- | --- | --- | --- |
| NN layer weight init (`Dense`, conv, recurrent, ...) | `.with_random_state(seed)` | Yes | Re-runs Xavier/Glorot uniform init. `Embedding` re-runs a uniform draw over `[-0.05, 0.05]` instead. Call it *before* training. See [3.2](../Chapter-03/3.2._Dense_Layers_and_Activations.md). |
