...
The gate tied most obviously to one machine is `cache_resident_max_bytes`. Set it to the L3 that 1 core reaches directly, and never to a package total: each sweep runs inside its own rayon task, and on a multi-die part another die's slice costs a fabric round trip close to DRAM. The default, 32 MiB, is the per-die figure for the calibration machine, whose 64 MiB of L3 comes in 2 instances. The band around it is still uncalibrated.
...
This table also shows why a calibration is worth rerunning. `CONV_PARALLEL_MIN_FLOPS` ships at 4,000,000, which an earlier run put inside its bracket. The serial convolution path then got more than twice as fast, which moves a crossover to the right, and the constant now sits below the bracket instead of inside it. A gate is calibrated against the code as it was on the day it was measured.
...
The takeaway line reads the ladder under 2 rules. Rungs that share a work value collapse to the slowest of them, because a crossover claim has to hold for every shape that reaches that work. Above the crossover, 1 rung may fall back to a loss without cancelling it, and the takeaway names that rung. The second rule matters: a largest shape whose working set overflows the cache falls to memory bandwidth and returns about 1.00x on any thread count. Read such a rung as a statement about bandwidth, never about scheduling.

Serial pooling speed depends strongly on the channel count. Window geometry gets amortized across channels. So the 25,088-tap `1x28x28x32` rung runs 14.8 us serial, while the *smaller* 16,384-tap `64x16x16x1` rung runs 77.7 us. Tap count alone does not predict serial cost. This is also why the takeaway collapses tied work values to their slowest rung.

`POOL_PARALLEL_MIN_OPS` is the worked example. It sits at 12,000, just under the bracket the bench now reports for it, and the 2 agree. They did not always: an earlier takeaway rule keyed on the *last* losing rung, which pushed the reported bracket past the 16,384-tap win above and hid it. **Calibration output is evidence, not an instruction.** Check which rungs of the ladder your workload actually sits on before you move a constant to match a bracket.
