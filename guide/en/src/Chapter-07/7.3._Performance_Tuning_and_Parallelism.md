...
Above the gate, the worker count ramps with the total work, not with any single dimension. gemmkit assigns 1 worker per `par_mnk_per_worker` (2,000,000 by default) of `m * n * k`. The count is floored at 1 and capped by the core count and the job count. Exact crossover numbers are machine-specific. Inspect the current values through `tuning::matmul::backend`, or measure your own machine with the `gemmkit-tune` autotuner (see [7.3.5](#735-recalibrating-the-2-tools-then-the-setters)).
...
gemmkit's blocking and job order do not depend on the worker count. For a fixed machine and configuration, the same product reproduces the same result *bit-for-bit*, no matter how many threads ran it. Re-running it is also deterministic.
...
| `set_/get_cache_resident_max_bytes` | 33_554_432 | bytes of L3 that 1 core reaches directly, for the per-row-GEMV-swarm vs tiled-GEMM decision |
...
| `set_/get_exp_map_f32` | 131_072 | f32 exp-dominated maps: the **forward** passes of sigmoid, tanh, softmax, ELU, SELU, softplus, exponential. Most matching backward passes reuse the cached activation, call no `exp`, and sit in the cheap-map class instead |
...
The gate tied most obviously to one machine is `cache_resident_max_bytes`. Set it to the L3 that 1 core reaches directly, and never to a package total. Each sweep runs inside its own rayon task. On a multi-die part, another die's slice costs a fabric round trip close to DRAM. The default, 32 MiB, is the per-die figure for the calibration machine, whose 64 MiB of L3 comes in 2 instances. The band around it is still uncalibrated.
...
## 7.3.5. Recalibrating: the 2 tools, then the setters
...
# Prints the tables and writes target/parallel_gates/RESULTS.md.
# Set RUSTYML_WRITE_RESULTS=1 to refresh the tracked benches/calibrations/RESULTS.md instead.
...
Here is a real excerpt from the checked-in `benches/calibrations/RESULTS.md`. It was regenerated on 2026-08-21, on the 9950X at 32 rayon threads:
...
This table also shows why a calibration is worth rerunning. `CONV_PARALLEL_MIN_FLOPS` ships at 4,000,000, below the measured bracket. A change to the code can shift the serial cost or the parallel cost, and move the crossover. The shipped constant does not follow it on its own. A gate is calibrated against the code as it was on the day it was measured.

Read the table the way its takeaway line does. The gate constant belongs at the crossover, with a small safety margin toward serial. A large penalty on a small tensor, to win a little on a medium one, is a bad trade. The ladder makes this asymmetry visible. At 84,672 FLOPs, the parallel path runs at under half the speed of serial (0.40x). At 8,294,400 FLOPs, just below the crossover, it still trails serial (0.85x). Only past the crossover, at 35,426,304 FLOPs, does parallel pull ahead, at 1.51x.

There is a trap here. The bench reports 1 work-estimate number per rung, but the serial cost of a kernel is not always a function of that number alone.

The takeaway line reads the ladder under 2 rules. Rungs that share a work value collapse to the slowest of them, because a crossover claim has to hold for every shape at that work value. Above the crossover, 1 rung may fall back to a loss without cancelling it, and the takeaway names that rung. The second rule matters: a largest shape whose working set overflows the cache falls to memory bandwidth and returns about 1.00x on any thread count. Read such a rung as a statement about bandwidth, never about scheduling.

Serial pooling speed depends strongly on the channel count. Window geometry gets amortized across channels. So the 25,088-tap `1x28x28x32` rung runs 15.1 us serial, while the *smaller* 16,384-tap `64x16x16x1` rung runs 80.4 us. Tap count alone does not predict serial cost. This is also why the takeaway collapses tied work values to their slowest rung.

`POOL_PARALLEL_MIN_OPS` is the worked example. It sits at 12,000, just under the bracket the bench now reports for it, and the 2 agree. A single named crossover cannot always speak for a ladder that has more than 1 local win. The takeaway line is a summary, and not the full evidence. **Calibration output is evidence, not an instruction.** Check which rungs of the ladder your workload actually sits on before you move a constant to match a bracket.
...
fn main() {
    ...
    tuning::matmul::set_cache_resident_max_bytes(16 * 1024 * 1024); // a 16 MiB per-die L3
...
Third, if you lean on the tiled-product paths (KNN, t-SNE, MeanShift), set `cache_resident_max_bytes` to the L3 1 core of your CPU reaches directly. Read it off `lscpu`: divide the L3 total by the number of instances.
