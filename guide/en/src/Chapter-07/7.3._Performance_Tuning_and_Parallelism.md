...
The `matmul` submodule has a distinct shape. It owns 2 gates of its own, plus `backend`, a `pub use gemmkit::tuning` re-export. Every `GEMMKIT_*` knob is reachable through a `set_*`/getter pair, with no direct `gemmkit` dependency in your `Cargo.toml`. The getters under `backend` use bare names, for example `parallel_threshold()`, not the `get_`-prefixed style. That naming is gemmkit's own, not RustyML's facade.
