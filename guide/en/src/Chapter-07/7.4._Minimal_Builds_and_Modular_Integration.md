...
Every module feature enables `math`, and `math` enables the numeric backend crates without condition. So no RustyML build exists without `ndarray`, `ahash`, `rayon`, `gemmkit`, and `gemmkit-ndarray`. Enabling any module feature adds all 5 of those crates. The heavier features add serialization and RNG machinery on top. `Cargo.toml`'s optional-dependency list gives this per-feature dependency set:
