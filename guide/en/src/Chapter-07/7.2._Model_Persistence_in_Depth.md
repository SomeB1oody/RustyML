...
fn main() {
    ...
    let file = ModelCheckpoint {
        ...
        layers: vec![LayerCheckpoint {
            ...
            build: None, // the reserved slot, which no layer fills today
...
Third, `build` is reserved and empty. Every layer allocates its arrays in its constructor today, so a live model already knows every extent and a file needs to carry none of them. The change that moves the allocation out of the constructors makes the build shape the 1 thing a fresh layer does not have, and a checkpoint is where it will come from. A load compares the field when the file and the layer both carry one, and skips the comparison in every other case.
