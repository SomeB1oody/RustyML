...
Both paths write [postcard](https://docs.rs/postcard), a compact, non-self-describing binary format. This 1 choice explains why the files are small and why only Rust can read them. It also explains why loading a file across a version boundary is a hazard.
...
`load_partial_from_path` is the lenient path, and a caller asks for it by name. It applies what matches and returns a `LoadReport { applied, missing, unused }` of checkpoint paths. Section [3.9.9](../Chapter-03/3.9._Saving_and_Loading_Weights.md#399-loading-part-of-a-file-on-purpose) works through it.
...
A classical model has no defense against this on its own. Its file carries no magic tag and no format version. A `Serialization` error from a size or type mismatch is the only warning a stale layout gives you. A change can also keep a field's type and position but give its stored value a new meaning, such as a different sign convention or a different label encoding. Such a change raises no error at all. The file loads, and the numbers are simply wrong.

The neural-network format defends against the first failure only. `MODEL_MAGIC` and `MODEL_FORMAT_VERSION` (see 7.2.3) let a build refuse a file from an incompatible format version before it applies even 1 weight. Bump `MODEL_FORMAT_VERSION` on any change to a record layout, a field order, or a field meaning. Every file from an earlier version then fails to load, with a message that names both versions. A classical file carries no such tag, so it has no comparable defense. Section 7.2.7 covers the sidecar pattern that closes this gap for classical models.
