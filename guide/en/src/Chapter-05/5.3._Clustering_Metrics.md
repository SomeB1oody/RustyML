...
## 5.3.1. 2 families of metric

The external metrics take 2 label arrays, `labels_true` and `labels_pred`, both of type `isize`. They measure how well the predicted partition reproduces the reference partition. Every metric on this page takes `isize` labels, the same type scikit-learn's `labels_` uses. This lets any clustering estimator in the crate feed any metric on this page. [KMeans](../Chapter-02/2.7._KMeans_Clustering.md), [Mean Shift](../Chapter-02/2.9._Mean_Shift.md), and [DBSCAN](../Chapter-02/2.8._DBSCAN.md) all return `Array1<isize>`, so no cast is ever needed. DBSCAN's and Mean Shift's `-1` noise label still needs attention (see [5.3.7](#537-handling-dbscans-noise-sentinel)), but it needs no type conversion. The internal metrics take the feature matrix `x` plus 1 label array. They score the partition against the geometry of the data, with no ground truth at all. Use internal metrics when you cluster unlabeled data, the usual case. Use external metrics when you have a reference partition and want to benchmark an algorithm against it.

| Function | Family | Inputs | Range | Perfect score | Chance-corrected |
|---|---|---|---|---|---|
| `adjusted_rand_index` | external | 2 `isize` label arrays | `[-0.5, 1.0]` | `1.0` | yes |
| `adjusted_mutual_info` | external | 2 `isize` label arrays | `[-1.0, 1.0]` typ. | `1.0` | yes |
| `normalized_mutual_info` | external | 2 `isize` label arrays | `[0.0, 1.0]` | `1.0` | **no** |
| `v_measure_score` | external | 2 `isize` label arrays | `[0.0, 1.0]` | `1.0` | no |
| `homogeneity_score` | external | 2 `isize` label arrays | `[0.0, 1.0]` | `1.0` | no |
| `completeness_score` | external | 2 `isize` label arrays | `[0.0, 1.0]` | `1.0` | no |
| `fowlkes_mallows_score` | external | 2 `isize` label arrays | `[0.0, 1.0]` | `1.0` | no |
...
`normalized_mutual_info` does **not** correct for chance. It only rescales MI into `[0.0, 1.0]`. It divides MI by the arithmetic mean of the 2 clusterings' entropies, `(H_true + H_pred) / 2`. This normalizer is a fixed convention in RustyML. There is no `average_method` switch like the one in scikit-learn. With this normalizer, NMI is numerically identical to `v_measure_score`, the harmonic mean of homogeneity and completeness. This gives a rule of thumb. When the 2 clusterings have different numbers of clusters, use `adjusted_rand_index` or `adjusted_mutual_info`. Reserve NMI and V-measure for comparisons at a fixed K, where the uncorrected bias stays constant and cancels out.

The 3 functions share the same signature. Only the meaning of the score differs.
...
The crossover point is a plain constant, and it is the 1 gate in the crate with no runtime setter. The parallel fill matches the serial fill numerically but not bit for bit, so a setter there would change the score this function returns. Every gate the tuning facade does expose selects a strategy only, and never changes a result. [7.3. Performance Tuning and Parallelism](../Chapter-07/7.3._Performance_Tuning_and_Parallelism.md) covers that promise and the reasoning behind parallel gates in general.
