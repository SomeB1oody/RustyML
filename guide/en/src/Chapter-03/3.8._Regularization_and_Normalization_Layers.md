...
Use the "depends on batch?" column as the practical guide for choosing a layer. Batch norm couples samples together through shared statistics. This makes it effective, but also fragile at small batch sizes. The other 4 layers normalize each sample independently, so batch size does not affect them, unless you point `UnitNormalization` at the batch axis. All 5 layers preserve the input shape. The shape guards below are not optional. Group and instance normalization require rank 3 or higher (a 2-D input returns `Error::InvalidInput`). Batch norm accepts rank 2 and higher.
...
`UnitNormalization` scales each group of elements to an L2 norm of 1. It divides by a length and subtracts no mean, so it changes how long each group is and never which way it points. That is what makes it the layer for cosine similarity, for a metric or retrieval head, and for anywhere a downstream dot product has to read as an angle. It is also the only layer on this page with no parameter, no `epsilon` argument, and no dependence on training mode.

`UnitNormalization::new(axis)` takes 1 argument, a `UnitNormalizationAxis`. `Default` is the last axis, which is Keras' `axis=-1` and, under the channels-last layout, the channel axis. `Custom(a)` names 1 axis, counting from 0 with the batch axis included. `Multiple(axes)` takes 1 norm over the combined elements of several axes, which need not be next to each other. The constructor rejects an empty list and a repeated axis with `Error::InvalidParameter`. Keras accepts an empty list, and it builds a layer with a repeated axis but then fails inside the reduction at the first call. An axis past the end of the input cannot be caught at construction, because the constructor sees no tensor, so it is an `Error::InvalidInput` at the first forward pass. So is a rank below 2.

The scale is `1 / sqrt(sum of squares)`, capped at `1e12`. The cap is Keras' `1 / epsilon` at its fixed `epsilon` of `1e-12`, and it is what decides the answer for a group that is all zero. Such a group is multiplied by `1e12` and stays all zero, rather than dividing by zero. A group whose sum of squares overflows `f32` goes the other way. Its reciprocal is `0`, so the group also comes back all zero.

One gradient differs from Keras, deliberately. Keras builds its gradient by automatic differentiation, and the reciprocal square root it differentiates through overflows at a group the cap took over, so Keras returns `NaN` for every element of that group. This layer returns the derivative of the function it actually computed, which is the cap itself. Everywhere else the 2 agree.
...
Which axes you pick decides how the layer reads the data. When they are the trailing block of the shape, which the default axis always is, each group is 1 contiguous run and the layer walks equal-length rows. That path costs 2 linear walks of the input, 1 for the norm and 1 for the scale. Any other choice leaves the groups as strided lanes, and the layer reduces them in place rather than paying for a transpose in and a transpose back out, at the price of 1 more tensor of the input size while it folds the squares. Both paths give the same numbers.
