...
`BatchNormalization` is the 1 layer in the crate that reports both kinds of parameter. For `C` channels it reports `2 * C` trainable elements, which are `gamma` and `beta`, and `2 * C` non-trainable elements, which are the running mean and the running variance. `param_count()` therefore returns `ParamCounts::new(2 * C, 2 * C)`, and `summary` prints a total of `4 * C` for the layer, split evenly between its 2 columns. The running statistics move on every training forward pass, but no optimizer ever touches them. This is the same split that Keras reports for this layer.
...
`UnitNormalization` and `Rescaling` also serialize as empty, and neither holds any state to lose. `LayerNormalization`, `GroupNormalization`, and `InstanceNormalization` serialize only their `gamma` and `beta` values. This is complete, because they recompute statistics from each input.
