...
`Valid` is the 1 mode that needs the kernel to fit the input axis. `Same` supplies the missing cells, so an effective kernel longer than the input axis stays legal there. The rule therefore runs in the forward pass and not in a constructor, because `with_padding` and `with_dilation_rate` both run after construction and both decide whether the rule applies. A `Conv2D` with a `3x3` kernel over a `2x2` input builds under every mode. It then returns `Error::InvalidInput` from `forward` under `Valid`, and it runs under `Same`. Section 3.5.10 shows both halves.
...
| `input_shape` of the wrong rank, or with zero channels (constructor) | `Error::InvalidInput` |
| `forward` handed a tensor of the wrong rank | `Error::InvalidInput` |
| Plain `Valid` convolution whose runtime spatial dim is below the effective kernel | `Error::InvalidInput` |
...
2 of these error paths need more explanation. The first is the kernel that does not fit its input axis. The engine checks it in the forward pass only, and only under `Valid` padding. A constructor cannot check it, because `with_padding` and `with_dilation_rate` both run after the constructor, and both decide whether the rule applies at all. Under `Valid` the engine computes `in - keff` in `usize`. Rather than let that computation underflow, it returns `InvalidInput`. Under `Same`, and under `Causal` on a `Conv1D`, the padding supplies the missing cells, so the same layer runs and returns the size those rules give.
...
fn main() {
    // 1. A kernel larger than the input builds. Only Valid padding refuses it, at forward time.
    let mut small_input =
    ...
    // The same layer under Same padding runs, because the padding supplies the missing cells.
    ...
    // 4. A runtime tensor smaller than the kernel: Valid geometry returns an error, not a panic.
    ...
    // 5. DepthwiseConv2D turns a runtime channel mismatch into a recoverable error.
    let mut dw =
