...
`keff` is the effective kernel of section 3.5.2, so a dilation of 1 reads these rules with the plain kernel size. The constructors mirror the plain ones argument for argument, and the 3 builder methods `with_padding`, `with_dilation_rate`, and `with_random_state` refine them:
