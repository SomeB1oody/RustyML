...
If you know Keras, you already know the layouts and weight shapes here: tensors are channels-last, and kernels carry their taps first. The constructor takes the configuration of the layer and nothing else. It never takes an input shape, because a kernel extent comes from the input and the input is not there yet. `Layer::build` reads the shape, sizes every array, and draws the weights. The model build calls it once per layer.
