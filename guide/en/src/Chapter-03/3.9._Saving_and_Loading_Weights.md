...
Layer, instance, and group normalization compute their statistics from the current input on every forward pass, so they hold no moving state. `gamma` and `beta` are all these layers need to save. [3.9.4](#394-normalization-running-statistics-survive-the-round-trip) shows the `BatchNormalization` case in detail.
...
fn main() -> Result<(), Error> {
    ...
    // The 2 prediction tensors must agree element-wise.
...
The difference would surface later, as a wrong prediction. So the load refuses on any disagreement, and the lenient path is the separate `load_partial_from_path`, which a caller asks for by name. [3.9.9](#399-loading-part-of-a-file-on-purpose) covers it.
