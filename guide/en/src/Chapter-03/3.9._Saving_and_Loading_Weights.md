...
The file also holds **the shape each layer was built for**. A layer allocates its arrays in `Layer::build`, from the shape of its input, so that shape decides every extent the layer holds. Recording it lets a load refuse a model that was built for another input, and the refusal names the layer and both shapes.

The batch axis of that record is free. A layer serves every batch size, so a checkpoint written by a model built for 1 sample loads into a model built for 32. A layer that owns no array and reads no extent of its input, such as an activation or `Dropout`, carries no build record at all, and a load skips the comparison for it.

| Persisted in the file | Not persisted |
| --- | --- |
| Per-layer type-name tag, and the build shape (both validated on load) | Activation functions, hyperparameters (`epsilon`, `momentum`, stride, ...) |
...
The full loop has 5 steps: build, train briefly, save, rebuild the *same* stack, and load. The last step confirms that the restored model predicts the same values. Note the `make_arch` function. It defines the architecture once, the input shape included, and both the live model and the reload target call it. This habit is the most useful practice for weights-only persistence, because it guarantees that the 2 stacks cannot drift apart. The build shape now travels in the file as well, so a drift in that 1 number is caught on load rather than at the first prediction.
...
`ModelStructureMismatch` is the error you hit most often while you develop a model. The strict load runs 6 checks, in this order: the layer count, the layer type of each position, the build shape where both sides carry one, the number of arrays of a layer and the name of each one, the kind of each array, and the shape of each array. The first 5 read the model alone. The whole first pass writes nothing, so a refusal leaves the model exactly as it was, and a model never holds the arrays of 1 file next to the arrays of another.

The message names the checkpoint path, or the layer position for a whole-layer disagreement. The build-shape check is the one that usually fires first now, and it fires before any array is compared. A `Dense` model built for `(None, 4)`, loaded from a file a model built for `(None, 5)` wrote, fails with ``model structure mismatch: layer 0 (`Dense`) was built for input shape (None, 4), and the file records (None, 5)``. Matching the error is straightforward:
...
**Transfer weights between programs.** A training binary builds the architecture, trains the model, and calls `save_to_path`. A separate serving binary builds the *identical* architecture and calls `load_from_path`. Share the `make_arch` function through a common module or crate, so the 2 programs cannot disagree on the architecture or on the input shape it builds for. The type-name, count, and build-shape checks catch a drift, but only after a failed load. A shared constructor catches the same drift at compile time. The serving binary never needs to call `compile`, since it only calls `predict`.
