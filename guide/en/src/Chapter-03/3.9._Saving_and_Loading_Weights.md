...
| `Conv1DTranspose` / `Conv2DTranspose` / `Conv3DTranspose` | transposed convolution `weight` kernel and `bias` (the kernel carries its filter axis before its channel axis) |
| `SeparableConv1D` / `SeparableConv2D` | depthwise `weight` kernel, pointwise `weight` kernel, and `bias` |
| `DepthwiseConv1D` / `DepthwiseConv2D` | depthwise `weight` kernel and `bias` (no pointwise kernel) |
...
This guard depends on developer discipline. RustyML bumps the version whenever a weight container's tensor layout, rank, or field order changes. It also bumps the version for a change to the order of the `LayerWeight` variants, because postcard writes the variant index, not the variant name. A new layer therefore appends its variant at the end of that enum. Appending leaves every existing index alone, so an older file still loads and no bump is needed.
...
For more detail on the persistence machinery, see [7.2. Model Persistence in Depth](../Chapter-07/7.2._Model_Persistence_in_Depth.md). That section covers the `get_weights` inspection path, the `SerializableSequential` wrapper, and how RustyML applies weights back through downcasting.
