...
A layer computes, and it remembers nothing. `forward` and `backward` both take `&self` and a `&mut Ctx`. The context carries the training flag, the caches of the pass, the parameter gradients, and the non-trainable state that a training pass changes. A built model is therefore `Send` and `Sync`, and several threads can run inference against 1 model. Three things separate this framework from most others: the strict `f32` precision, the input shape that reaches a model 1 time at its build, and the `Result` that every constructor returns.
...
`Add` above belongs to the merge family, which a graph model needs and a chain model cannot use. The family has 7 layers: `Add`, `Subtract`, `Multiply`, `Average`, `Maximum`, `Minimum`, and `Concatenate`. The first 6 reduce their inputs element by element, and `Concatenate` joins them along 1 axis. [3.10](./3.10._Graph_Models_and_Merge_Layers.md) covers all 7, and the shape rule of each. `Average` is the 1 name that 2 categories of the crate give to an item. The root of the prelude therefore keeps `metrics::Average`. The merge layer is `prelude::neural_network::Average`, or `neural_network::layers::Average`.
