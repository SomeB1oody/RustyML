...
The result shape is whatever the last layer emits, `(batch, output_dim)` for a `Dense` tail on a rank-2 input. Input must match each layer's expectations. A `Dense` layer requires a tensor of rank 2 or more whose last axis is `input_dim` wide, and it returns `Error::InvalidInput` for anything else.
...
fn main() {
    ...
    model
        ...
        .add(Dense::new(8, 2, Activation::Softmax { axis: -1 }).unwrap().with_random_state(0))
        .compile(
...
| rank-1 input into a `Dense` layer, or a last axis that is not `input_dim` wide | `Error::InvalidInput(_)` |
...
model
    ...
    .add(Dense::new(8, 2, Activation::Softmax { axis: -1 }).unwrap()); // expects 8, a mismatch
