...
The [prelude](../Chapter-01/1.5._The_Prelude_and_Imports.md) exports everything on this page. It exports `SequentialBuilder`, `Sequential`, `History`, `Shape`, `Dense`, `Activation`, every optimizer and loss, and the `Tensor` alias. 1 glob is therefore the whole import list of every example below:
...
`evaluate` sits between the 2. It needs a loss to score with, and no optimizer to step with. This split matters for serving. After a [weight load](./3.9._Saving_and_Loading_Weights.md) into a fresh architecture, `predict` runs right away without a compiled optimizer.
