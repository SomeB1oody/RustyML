...
Section 3.7.1 covers both flags, and [3.7.9](#379-a-bidirectional-model) covers what to do about the pair. They carry no `return_state`, no `bidirectional` wrapper, and no in-cell dropout.
...
You may want to install weights by hand, for example to port from another framework or to unit-test an exact recurrence. Every layer has `set_weights(kernel, recurrent_kernel, bias)` for this. This method takes the fused matrices directly. LSTM and GRU also have `set_gate_weights(...)`. It accepts 1 `(kernel, recurrent_kernel, bias)` triple per gate, and concatenates the triples into the fused layout for you.
...
The example below learns a real sequence task: predict the sum of a length-3 scalar sequence. It uses an `LSTM` encoder, a `Dense` readout, `Adam`, and mean-squared error. It converges in a few hundred full-batch epochs. Recall from [3.1. The Sequential Model](./3.1._The_Sequential_Model.md) that `fit` runs 1 full-batch gradient step per epoch:
...
`Reverse::new(1)` puts the branch back into input order. Slot 0 of the backward branch then holds the state that consumed input timestep 0, which is what the forward branch holds there. See [3.5. Convolutional Layers](./3.5._Convolutional_Layers.md), section 3.5.8.
