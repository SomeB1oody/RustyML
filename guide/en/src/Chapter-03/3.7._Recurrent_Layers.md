...
`new` returns `Error::InvalidParameter` when `input_dim` or `units` is 0. RustyML has no `return_state` option, no `bidirectional` wrapper, and no in-cell dropout. Each layer uses one dense implementation.
...
`with_return_sequences(true)` on the lower layer removes the problem. That layer then emits `(batch, timesteps, units)`, which is the rank the next layer reads, and its `units` becomes the next layer's `input_dim`. Set the flag on every layer of a stack except the last one. The following program stacks 2 LSTM layers on the same task the next example uses:
...
`model.summary()` prints each recurrent layer's output shape as `(None, units)`, and as `(None, None, units)` under `return_sequences`. The first `None` stands for the dynamic batch dimension, and the second for the time axis, which the layer reads from the input rather than from a declared shape. For anything larger than a toy dataset, use `fit_with_batches` instead of `fit`, so each epoch takes several mini-batch steps and reshuffles the data.
...
The table is the only parameter, so `param_count()` reports `input_dim * output_dim`. It starts from a uniform draw over `[-0.05, 0.05]`, which is the Keras default for this layer, not the Xavier/Glorot rule that `Dense` uses. Call `.with_random_state(seed)` to make the draw reproducible. Read the table with `get_weights()`, which gives the `LayerWeight::Embedding` variant, and write it with `set_weights(table)`. Both use the `(input_dim, output_dim)` shape.
