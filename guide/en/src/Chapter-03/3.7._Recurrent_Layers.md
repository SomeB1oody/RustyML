...
**The `Reverse` is not optional.** The backward branch emits its states in processing order, so its slot `k` holds the state of input timestep `steps - 1 - k` while the forward branch slot `k` holds timestep `k`. Merging them without the reversal pairs mismatched timesteps at every position, and nothing reports it. `Reverse::new(1)` puts the branch back into input order, after which slot 0 of the backward branch holds the state that consumed input timestep 0, which is what the forward branch holds there. See [3.5. Convolutional Layers](./3.5._Convolutional_Layers.md), section 3.5.8.

With `return_sequences` left at false the reversal is unnecessary and `Reverse` would refuse the rank-2 input, because the branch returns 1 state per sample and that state already consumed input timestep 0.
...
**Do not share 1 layer between the 2 branches.** A graph may call 1 layer from several nodes, and that is real weight sharing, but `go_backwards` is a field of the layer and not an argument of the call. A shared layer therefore runs the SAME direction at both nodes, and the second branch carries no backward information at all. The model builds, predicts and trains, and nothing reports it.
