...
`RepeatVector` bridges that gap. It turns a `(batch, units)` state into a `(batch, n, units)` sequence, so a second recurrent layer can read it. Read what that buys you carefully. Every one of the `n` steps holds the same vector. The second layer therefore sees a constant sequence, not the per-timestep states of the first. That is the encoder-decoder pattern, in which a fixed context seeds the decoder. It is not Keras' `return_sequences=True`, and a deep stack in that sense stays out of reach. See [3.5. Convolutional Layers](./3.5._Convolutional_Layers.md), section 3.5.7, for the layer itself.
