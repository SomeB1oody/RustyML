...
[`gemmkit-ndarray`](https://crates.io/crates/gemmkit-ndarray) adapter. The `math` feature pulls in
both crates. RustyML keeps only a thin layer of its own code in `src/math/matmul.rs`.
...
The first 2 items are generic over `T: gemmkit::GemmScalar`. In RustyML's build, this means
...
it from the last-level cache size"). Below that floor, the touched data is LLC-resident. 1 core
already sees the full LLC bandwidth, so splitting the work only adds fork-join overhead and cache
contention.

Above the floor, the work splits across a memory-bandwidth worker cap, `gemv_thread_cap`, also `0`
by default for auto mode. DRAM saturates at far fewer workers than a machine has logical cores. A
third knob, `gemv_threshold`, caps how large the vector side may be before the shape falls back to
the general driver. Its default is `usize::MAX - 1`, effectively unbounded. In practice, a
...
general driver entirely, for gemmkit's gemv path. It gates instead on a byte floor derived from the
last-level cache. It also caps its worker count at what the memory system can actually feed. This
path is bandwidth-bound, so extra cores start to pay off far earlier there than in a compute-bound
GEMM, which can better amortize thread dispatch.
