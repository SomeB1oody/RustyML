...
| `gemm_chunk_rows(row_len)` | `pub(crate)` | `gemm_chunk_elems() / row_len`, clamped to `[16, 4096]` rows |
| `cache_resident::<T>(rows, cols)` | `pub(crate)` | whether `rows * cols * size_of::<T>()` is under `cache_resident_max_bytes()` |
...
that would otherwise have to materialize a pairwise projection too large to hold at once. All 4
items are crate-internal, so none of them is callable from outside. Use the
[6.2.5](#625-tuning-the-gates-the-public-surface) knobs that govern the policy instead.
...
- `SimpleRNN`, `LSTM`, and `GRU` project their inputs once with `dot`. Each timestep then fuses its
  recurrent projection with `gemm_fused`. All 3 drop to plain `gemm` for the recurrent-kernel
  gradient, which writes into a column block of a larger buffer.
...
environment variable beats the compiled default. gemmkit reads each environment variable once, on
the knob's first access, and caches it for the rest of the process.
...
gemmkit builds the tier pools once and reuses them warm. It does not rebuild them per call. A value
of `0` disables them entirely. The default is arch-split: 2 tiers on x86-64, 1 tier on aarch64, and
0 tiers on every other target, pending on-device validation.
...
L2-resident, and that core already sees the full L2 bandwidth. Splitting the work only adds
...
there. Cutting it gives every worker a strided walk over the whole matrix. A row-major matrix
...
them, because such a table would go out of date quickly. gemmkit's own docs.rs page documents
them. Each knob is reachable through `rustyml::tuning::matmul::backend`. The
...
- `matvec_serial_and_auto_agree_bitwise` covers the bandwidth-bound gemv path. It reduces each
  output element there over the whole of `k` on 1 worker.
...
version than the adapter's would give you a second copy. A `set_*` call on it would then have no
...
| `get_cache_resident_max_bytes` / `set_cache_resident_max_bytes` | 33,554,432 | cache-resident size threshold, set to the L3 that 1 core reaches directly |
| `matmul::backend::*` | see gemmkit | every backend knob, each with a matching `GEMMKIT_*` environment variable |

`cache_resident_max_bytes` is the knob you are most likely to change. Set it to the L3 that 1 core
of your machine reaches directly, and never to a package total. Each sweep runs inside its own
rayon task, so the slice local to the core running it is what counts. On a multi-die part, another
die's slice costs a fabric round trip close to DRAM. The default, 32 MiB, is the per-die L3 of the
calibration machine, whose 64 MiB comes in 2 instances. The band around it is not calibrated, and
the gate prices only the shared matrix, not the buffer the tiled branch allocates.
...
the activation run inside the kernel's epilogue, not as extra passes. A training context also parks an
input cache and an output cache for the backward pass, so an inference context times the
projection alone.
...
- A deep-`k` shape is the interesting one. Here `m` and `n` stay small while `k` grows. A common
  intuition says a skinny shape must be bandwidth-bound. A deep-`k` product is usually
  compute-bound instead, because its operands are small against the arithmetic they feed. This is
  the shape where depth-blocking decisions matter most.
...
core's private L2, then climbs a worker ladder as the bytes it touches grow. DRAM saturates
...
fn main() {
    ...
    // An inference context parks no cache, so the timing shows the GEMM alone.
    let (batch, fin, fout) = (256usize, 256usize, 256usize);
    let mut layer = Dense::new(fout, Activation::ReLU)
        .unwrap()
        .with_random_state(42);
    // `build` allocates the kernel and spends the recorded seed. `forward` takes
    // `&self`, so it cannot build, and it refuses an unbuilt layer.
    layer.build(&Shape::with_free_batch(&[batch, fin])).unwrap();
    ...
    let warm = layer.forward(&x, &mut ctx).unwrap();
...
The defaults and the shape fix the gate and the work count, so those print exactly. The
...
On a small product like this, or on a machine with few cores, the parallel run may not be
faster. That is the whole point of the gate, and it is why the defaults keep sub-gate
