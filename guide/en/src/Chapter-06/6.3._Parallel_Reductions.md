...
accumulation instead uses `n_samples * n_features`, because that product is the work each block
does. It is not the number of blocks. The fold splits the sample axis alone, so a wide, short
input can clear the gate on its feature count while the fold still has 1 task. Such a call site
pairs its product with a floor on the axis the fold actually splits: at least 2 blocks of
`DET_REDUCE_BLOCK`. The gate moves the crossover point, but it never touches correctness, because
the blocked fold gives the same answer on either side.
