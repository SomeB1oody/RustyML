...
- The first line means "use version 0.16 of the `rustyml` crate with its default features" (that is, with everything enabled), so all of RustyML is available to you.
...
rustyml = { version = "0.16", default-features = false, features = ["neural_network"] }
...
rustyml = { version = "0.16", features = ["full", "show_progress"] }
```
