...
Note the exact casing on `RMSprop` (lowercase `p`). Activations come in two forms. One is the standalone layers in the table above. The other is a variant of the `Activation` enum, such as `Activation::ReLU` or `Activation::LeakyReLU { negative_slope }`. Any layer that accepts an `Activation` also accepts a standalone activation layer, because each one implements `Layer` and converts `Into<Activation>`. [3.2. Dense Layers and Activations](../Chapter-03/3.2._Dense_Layers_and_Activations.md) lists the full set with the formula for each one.
