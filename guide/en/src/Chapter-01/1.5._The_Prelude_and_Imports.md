...
Unlike the error-propagation design in the rest of the crate, the metric functions panic outright on an error, which keeps the module lightweight. See [5. Model Evaluation](../Chapter-05/5.0._Model_Evaluation.md) for the metrics module in depth. Their argument order is `(y_true, y_pred)`, matching scikit-learn.
