...
When you use RustyML, do not seed `ndarray-rand`'s generator by hand. RustyML has its own randomness management (`set_global_seed`, a per-model `with_random_state`, and `SequentialBuilder::new_with_seed`), so control randomness through RustyML's API instead; see [7.1 Reproducibility and Random Seeds](../Chapter-07/7.1._Reproducibility_and_Random_Seeds.md).
