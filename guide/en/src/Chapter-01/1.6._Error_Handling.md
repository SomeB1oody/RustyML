...
`NotBuilt` is the newest of the 5. A layer allocates every array it owns in `Layer::build`, so a layer that never built holds no kernel and no bias. `Layer::predict` takes `&self` and cannot build, so it reports this variant. A model that comes from `SequentialBuilder::build` is always built, and it never raises it.
