...
fn test_gaussian_noise_new() {
    let noise_layer = GaussianNoise::new(0.1, vec![32, 128]);
...
fn test_gaussian_noise_negative_stddev() {
    GaussianNoise::new(-0.1, vec![32, 128]);
}

#[test]
fn test_gaussian_noise_forward_training() {
    let mut noise_layer = GaussianNoise::new(0.5, vec![4, 4]);
...
fn test_gaussian_noise_zero_stddev() {
    let mut noise_layer = GaussianNoise::new(0.0, vec![4, 4]);
    let input = Array2::ones((4, 4)).into_dyn();
...
fn test_gaussian_noise_backward() {
    let mut noise_layer = GaussianNoise::new(0.1, vec![4, 4]);
