...
//! blocking + SIMD) and adds the missing parallelism at one level: large products are split
//! into row or column blocks across rayon, each block computed by a serial `dot`, and
...
/// amortize that packing. Block sizes 16-64 sit on a plateau (within noise of each other) and
/// 128+ degrades, so 64 is kept, for both `f32` and `f64`. The packing economics barely depend
/// on the element type, so the constant is shared rather than per-type
...
/// samples of few features, where the split axis (the feature count) is small. The tall shape
/// plateaus over 8-64 rows and the short-wide shape over 1-16 (degrading from 32 up), so 8
/// takes the intersection
const PAR_GEMV_MIN_BLOCK: usize = 8;

/// Calibrated serial/parallel crossover thresholds for the crate-internal
/// [`gemm_internal`] / [`gemv_internal`] wrappers, per element type
///
/// The constants are machine-calibrated values and may be retuned in any release; the public
/// [`gemm`]/[`gemv`] take the threshold as a parameter instead, so this trait stays
/// crate-internal
#[cfg(any(
...
pub(crate) trait MatmulElem: LinalgScalar + Send + Sync {
    ...
    /// matrix once for O(1) FLOPs per element), so its crossover is a different cost class. Only
    /// the classical-ML/utils features have matvec call sites, hence the narrower cfg
...
impl MatmulElem for f32 {
    /// Measured crossover bracket is 2.1M-4.2M FLOPs (square and skinny shapes agree), so the
    /// gate sits at the proven-win end of the bracket
    const PAR_GEMM_MIN_FLOPS: usize = 4_000_000;

    /// Crossover bracket 131K-524K FLOPs (square/tall/short-wide shapes agree), gate at the
    /// proven-win end
    #[cfg(any(feature = "machine_learning", feature = "utils"))]
...
impl MatmulElem for f64 {
    /// Crossover bracket 524K-1.77M FLOPs (the win at 1.77M is a thin 1.14x), so the gate sits
    /// just above the bracket. Lower than `f32`'s 4M, as expected from the halved SIMD width
    const PAR_GEMM_MIN_FLOPS: usize = 2_000_000;

    /// Crossover bracket 131K-524K FLOPs, same bracket as `f32` (the matvec is bandwidth-bound
    /// either way), gate at the proven-win end
    #[cfg(any(feature = "machine_learning", feature = "utils"))]
    const PAR_GEMV_MIN_FLOPS: usize = 524_288;
}
...
/// threshold is a pure performance knob: pass the crossover measured on your machine, `0` to
/// always split, or `usize::MAX` to always run serial
///
...
/// [`gemm`] with the calibrated crossover for `T` ([`MatmulElem::PAR_GEMM_MIN_FLOPS`]) as the
/// threshold - the crate-internal call form
#[cfg(any(
    feature = "machine_learning",
    feature = "neural_network",
    feature = "utils"
...
/// [`gemv`] with the calibrated crossover for `T` ([`MatmulElem::PAR_GEMV_MIN_FLOPS`]) as the
/// threshold - the crate-internal call form
#[cfg(any(feature = "machine_learning", feature = "utils"))]
...
/// fewer chunks means less DRAM traffic. Measured over the GEMV swarm (500k x 64 training set):
/// 8M elements -> 1.23x, 32M -> 1.72x, 64M -> 1.93x. The budget caps the transient chunk buffer
...
/// a tiled GEMM (which streams `X` once per chunk) wins instead. At 25 MB the swarm wins ~2x;
/// at 256 MB the tiled GEMM wins ~2x. Only those two points bracket the boundary, so the
/// constant sits at the full 64 MB L3 size; the band between is uncalibrated
...
/// amortize that packing. On f32/f64, 16-64 sit on a plateau and 128+ degrades; pass 64 when
/// in doubt - [`gemm`] uses exactly that. Like the FLOPs threshold, `min_block` is a pure
/// performance knob: splitting only the `m`/`n` axes leaves every output element's
/// `k`-accumulation order untouched, so the result is **bitwise identical** to the serial
/// `a.dot(&b)` at any block size and any thread count
///
...
/// than a GEMM's. On f32/f64, tall shapes plateau over 8-64 rows and short-wide ones over
/// 1-16; pass 8 when in doubt - [`gemv`] uses exactly that. Like the FLOPs threshold,
/// `min_block` is a pure performance knob: splitting only the row axis leaves every output
/// element's `k`-accumulation order untouched, so the result is **bitwise identical** to the
/// serial `a.dot(&x)` at any block size and any thread count
///
/// # Parameters
///
/// - `a` - Matrix with shape (m, k); any storage (owned array, view, transpose)
