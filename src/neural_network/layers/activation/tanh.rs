...
//!
//! The `Tanh` struct holds only the shape recorded at build time, because the layer takes no
//! parameter. `UnaryLayer::forward` computes `Activation::Tanh` and caches the output during
