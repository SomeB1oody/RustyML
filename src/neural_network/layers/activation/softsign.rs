...
//!
//! The `Softsign` struct holds only the shape recorded at build time, because the layer takes
//! no parameter. `UnaryLayer::forward` computes `Activation::Softsign` and caches the output
