...
//!
//! Everything works directly on the channels-last buffer. A channel group is a contiguous
//! sub-run of each position's channel vector, never a plane gathered out separately.
...
///
/// The block size depends only on the input shape. The block grouping, and with it the float
/// summation order, therefore never depends on how many threads ran
fn positions_per_block(channels: usize) -> usize {
    rows_per_block(channels)
}

/// Per-group `(shift, sum, sum_of_squares)` accumulators for 1 batch item
struct GroupStats {
...
/// dwarfs the spread.
///
/// Blocks merge in block order, so the result depends on the shape, not on the schedule
fn group_stats(x: &[f32], b: usize, layout: &GroupLayout, parallel: bool) -> GroupStats {
    ...
    // The shift broadcast to 1 value per channel, so the block fold below is a flat pass
...
/// sweep below into a flat elementwise pass over contiguous rows. It needs no per-element group
/// lookup and no modulo, and both operands line up directly with `gamma`/`beta`
fn per_channel_stats(
    ...
    for gr in 0..layout.num_groups {
        let m = stats.s1[gr] / n;
        // Rounding can leave the difference a hair below zero on a near-constant batch item. The
        // clamp keeps the square root real without changing a well-conditioned result
...
/// The slabs for 1 batch item in the forward sweep: `(x_normalized, output)` to fill from `input`
type ForwardSlabs<'a> = (usize, ((&'a mut [f32], &'a mut [f32]), &'a [f32]));

/// The slabs for 1 batch item in the backward sweep: `grad_input` to fill from
/// `(grad_output, x_norm)`
...
///
/// Runs 2 linear passes over each batch item. [`group_stats`] computes the per-group statistics,
/// then 1 sweep writes `x_normalized` and the affine output. Both passes walk contiguous
...
///
/// Inverse of [`group_norm_forward_core`]. The parameter gradients are the same column folds
/// the rest of the module uses. Under this layout `[batch, spatial..., channels]` is already
/// the `[M, C]` matrix they read. The input gradient takes 2 passes per batch item: the
/// per-group reductions, then the fused composition below.
///
...
pub(super) fn group_norm_backward_core(
    ...
    // Each batch item runs in its own task. Their slabs are disjoint, so nothing needs a merge
...
mod tests {
    ...
    ///
    /// `[1, 2, 4]` is 1 sample with 2 positions and 4 channels split into 2 groups. Group 0 sees
    /// {1, 2, 3, 4} (mean 2.5, variance 1.25) and group 1 sees {10, 20, 30, 40} (mean 25, variance
    /// 125). The 2 groups have wildly different scales. A group boundary read anywhere but
    ...
    ///
    /// This is the case the shifted sum exists for. The data sits around `1e6` with a spread of
    /// a few units, so `E[x^2]` is about `1e12`, past f32's about 7 significant digits. The
    /// textbook `E[x^2] - E[x]^2` form would subtract 2 nearly equal huge numbers there and
    /// return garbage or a negative variance. Measuring deviations from an element of the data
    /// keeps both accumulators on the order of the spread. The normalized output is still the
    /// plain z-score
    #[test]
    ...
    /// The group count spans 2 extremes. 1 group across every channel reduces to per-sample
    /// normalization, and 1 group per channel reduces to per-channel normalization
    #[test]
    fn group_norm_group_count_extremes() {
        ...
        // 2 groups (1 channel each): channel 0 is {1, 3, 5} and channel 1 is {2, 4, 6}
