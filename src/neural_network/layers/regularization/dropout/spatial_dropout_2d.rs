...
/// let mut spatial_dropout = SpatialDropout2D::new(0.2).unwrap();
///
/// // Create input tensor (batch_size=32, height=28, width=28, channels=64)
...
impl LayerBase for SpatialDropout2D {
...
impl UnaryLayer for SpatialDropout2D {
    /// Records the shape the layer serves. The layer holds no array, so nothing is
    /// allocated
    fn build(&mut self, input: &Shape) -> Result<(), Error> {
        let Some(built) = start_build(&self.built, "SpatialDropout2D", input)? else {
            return Ok(());
        };
        input.check_rank("SpatialDropout2D", 4)?;
        self.built = Some(built);
        Ok(())
    }

    /// Drops whole channels of a `(batch_size, height, width, channels)` tensor
    ///
    ...
    fn forward(&self, input: &Tensor, ctx: &mut Ctx) -> Result<Tensor, Error> {
        ...
        ctx.push_cache("SpatialDropout2D", mask_2d);
