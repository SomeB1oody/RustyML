...
//!
//! [`SpatialDropout1D`] holds the drop rate, the shape recorded by
//! [`UnaryLayer::build`](crate::neural_network::traits::UnaryLayer::build), and the random number
//! generator that draws the per-channel mask. [`SpatialDropout1D::with_random_state`] reseeds that
...
impl UnaryLayer for SpatialDropout1D {
    ...
    fn forward(&self, input: &Tensor, ctx: &mut Ctx) -> Result<Tensor, Error> {
        ...
        ctx.push_cache("SpatialDropout1D", mask_2d);
