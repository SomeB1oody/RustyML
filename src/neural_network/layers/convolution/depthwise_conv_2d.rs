...
//!
//! [`DepthwiseConv2D`] gives each input channel its own kernel or kernels, with no mixing
//! between channels. `DepthwiseConv2DCache` holds the tensors the layer parks between its
...
///
/// // DepthwiseConv2D with ReLU. The build draws the weights
...
impl DepthwiseConv2D {
    ...
    /// [`DepthwiseConv2D::with_padding`]. The kernel is solid by default. Space its taps out with
    /// [`DepthwiseConv2D::with_dilation_rate`]. The layer seeds weights from the global seed or
    /// entropy by default. For reproducible initialization, set a seed with
...
/// What the forward pass of [`DepthwiseConv2D`] parks for its backward pass
...
impl UnaryLayer for DepthwiseConv2D {
    /// Reads the channel count from the input shape, and draws the kernel and the bias
    fn build(&mut self, input: &Shape) -> Result<(), Error> {
        let Some(built) = start_build(&self.built, "DepthwiseConv2D", input)? else {
            return Ok(());
        };
...
mod tests {
    ...
    fn depthwise_forward_keeps_channels_separate_hand_derived() {
        let mut layer = DepthwiseConv2D::new((2, 2), (1, 1), Linear::new()).unwrap();
