...
/// \[batch_size, output_height, output_width, filters\]. This is the layer a decoder uses to
/// reach the resolution the matching [`Conv2D`](super::conv_2d::Conv2D) consumed.
///
...
/// axis's kernel size after dilation, defined in [`Conv2DTranspose::with_dilation_rate`]:
///
...
pub struct Conv2DTranspose {
    ...
    /// Stride values for the transposed convolution as (height, width)
    strides: (usize, usize),
    /// Tap spacing of the kernel as (height, width). 1 gives a solid axis
...
impl Conv2DTranspose {
    ...
    /// - `strides` - Stride values for the transposed convolution as (height, width)
    ...
    /// [`Conv2DTranspose::with_padding`]. The kernel is solid by default. Space its taps out with
    /// [`Conv2DTranspose::with_dilation_rate`]. By default, the layer seeds weights from the global
...
/// What the forward pass of [`Conv2DTranspose`] parks for its backward pass
struct Conv2DTransposeCache {
...
impl LayerBase for Conv2DTranspose {
...
impl UnaryLayer for Conv2DTranspose {
    ...
    fn backward(&self, grad_output: &Tensor, ctx: &mut Ctx) -> Result<Tensor, Error> {
        let cache: Conv2DTransposeCache = ctx.pop_cache("Conv2DTranspose")?;
