...
//! `model.predict(..)`); the trait implementations simply forward to them, and Rust's
...
mod tests {
    ...
    fn fit_and_predict_through_traits() {
        // Exercises the trait methods to confirm they forward to the inherent
        // methods (and therefore do not recurse) and produce correct results.
        let x = Array2::from_shape_vec((3, 1), vec![1.0, 2.0, 3.0]).unwrap();
        let y = Array1::from_vec(vec![2.0, 4.0, 6.0]);
