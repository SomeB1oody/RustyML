...
/// use rustyml::machine_learning::linear_discriminant_analysis::{LDA, Shrinkage, Solver};
///
...
impl LDA {
        ...
        S2: Data<Elem = i32>,
    {
        ...
        let cov_inv = self.solver.invert_covariance(&cov)?;
        ...
        let solver_matrix =
            self.solver
                .discriminant_matrix(&cov, sb, self.cov_inv.as_ref().unwrap())?;
        let projection = self.solver.project(&solver_matrix, self.n_components)?;
